...
#include <deep_gemm/common/asymScheduler.cuh>
...
sm100_fp8_asym_gemm_1d1d_impl(uint32_t* offsets, uint32_t* experts,
    ...
    const uint32_t shape_sf_k = ceil_div_device(shape_k, BLOCK_K * kNumSFStagesPerLoad);
    ...
    // todo: Shutian
    constexpr uint32_t SMEM_C_SIZE_PER_STAGE = LOAD_BLOCK_N * BLOCK_K * sizeof(__nv_fp8_e4m3);
    ...
    // todo: Shutian
    auto smem_c  = PatternVisitor([&](const uint32_t& i) {
        return reinterpret_cast<cutlass::float_e4m3_t*>(smem_buffer + SMEM_CD_SIZE + kNumStages * SMEM_A_SIZE_PER_STAGE * 2 + i * SMEM_B_SIZE_PER_STAGE);
    });
    ...
    // ---- Single extra barrier(s) ----
    auto barrier_b                 = PatternVisitor([=](const uint32_t& i) { return barrier_start_ptr + (kNumStages * 3 + kNumEpilogueStages * 2); });
    auto empty_barrier_b           = PatternVisitor([=](const uint32_t& i) { return barrier_start_ptr + (kNumStages * 3 + kNumEpilogueStages * 2 + 1); });
    auto with_sf_b_barrier         = PatternVisitor([=](const uint32_t& i) { return barrier_start_ptr + (kNumStages * 3 + kNumEpilogueStages * 2 + 2); });

    constexpr uint32_t kBaseBarriers = 3 * kNumStages + 2 * kNumEpilogueStages;

    // ---- Single extra barrier(s) ----
    // auto barrier_b                  = PatternVisitor([=](const uint32_t& i) { return barrier_start_ptr + (kNumStages * 3 + kNumEpilogueStages * 2 + 1); });
    // auto empty_barrier_b            = PatternVisitor([=](const uint32_t& i) { return barrier_start_ptr + (kNumStages * 3 + kNumEpilogueStages * 2 + 2); });


    // Barrier* barrier_b       = barrier_start_ptr + kBaseBarriers;       // 1 barrier
    // Barrier* empty_barrier_b = barrier_start_ptr + (kBaseBarriers + 1); // optional 2nd barrier

    // ---- Place tmem pointer AFTER the extra barriers ----
    constexpr uint32_t kExtraBarriers = 3; // set to 1 if you only add barrier_b
    auto tmem_ptr_in_smem = reinterpret_cast<uint32_t*>(
        barrier_start_ptr + kBaseBarriers);
    ...
    if (warp_idx == 1 and cute::elect_one_sync()) {
        ...
        // ---- Initialize barrier for Matrix W ----
        barrier_b[0]->init(1);
        empty_barrier_b[0]->init(1);  // if you use it
    ...
    uint32_t n_idx = scheduler.n_idx * BLOCK_N;
    // Dispatch warps into different roles
    if (warp_idx == 0 and cute::elect_one_sync()) {
        // Todo: batch_idx
        constexpr bool kIsBatchedMM = (kGemmType == GemmType::Batched);
        const uint32_t batch_idx = 1;

        // printf("blockIdx.x: %d, n_idx: %d, scheduler.blocks_perExpert: %d, scheduler.m_start: %d, scheduler.m_end: %d\n", blockIdx.x, n_idx, scheduler.blocks_perExpert, scheduler.m_start, scheduler.m_end);
        ...
        for (int block_k_iter = 0; block_k_iter < block_k; ++block_k_iter) {
            ...
            tma_copy<LOAD_BLOCK_N, BLOCK_K, kSwizzleBMode, cutlass::float_e4m3_t, kIsBatchedMM>(
                &tensor_map_b, barrier_b[0], smem_b[stage_idx], n_idx, k_idx, 1, batch_idx);
            auto num_arrival_bytes_b = SMEM_B_SIZE_PER_STAGE;

            const uint32_t sf_stage_in_group_idx = block_k_iter % kNumSFStagesPerLoad;
            if (sf_stage_in_group_idx == 0) {
                uint32_t sf_stage_idx = scheduler.n_idx * block_k +  block_k_iter;
                tma_copy<BLOCK_N, 1, 0>(&tensor_map_sfb, full_barriers[stage_idx], smem_sfb[stage_idx], n_block_idx * BLOCK_N, sf_stage_idx);
                num_arrival_bytes_b += BLOCK_N * sizeof(uint32_t);
            }

xx                // Wait consumer release
                empty_barriers[stage_idx]->wait(phase ^ 1);           
                int m_idx = block_m_iter * BLOCK_M;
                ...
                // Todo: include cute::UMMA::Major::K
                ...
                tma_copy<LOAD_BLOCK_M, BLOCK_N, kSwizzleAMode, cutlass::float_e4m3_t, kIsBatchedMM>(
                    &tensor_map_cd, full_barriers[stage_idx], smem_c[stage_idx], m_idx, n_idx, 1, batch_idx);
                
                auto num_arrival_bytes = SMEM_A_SIZE_PER_STAGE + SMEM_C_SIZE_PER_STAGE;

                // Issue SFA and SFB TMAs at certain stages
                // No swizzling, so one TMA for one SF is enough
                const uint32_t sf_stage_in_group_idx = block_k_iter % kNumSFStagesPerLoad;
                if (sf_stage_in_group_idx == 0) {
                    uint32_t sf_stage_idx = block_k_iter +  block_m_iter * block_k;
                    // Todo: check the sf_stage_idx
                    tma_copy<BLOCK_M, 1, 0>(&tensor_map_sfa, full_barriers[stage_idx], smem_sfa[stage_idx], m_block_idx * BLOCK_M, sf_stage_idx);
        ...
        for (int block_k_iter = 0; block_k_iter < block_k; ++block_k_iter) {
            ...
            auto empty_barrier_arrive = [&](const bool& do_tmem_full_arrive) {
                ...
                umma_arrive(reinterpret_cast<uint64_t*>(empty_barrier_b[0]));
            };

            // Empty barrier arrival
            auto empty_barrier_arrive_b = [&](const bool& do_tmem_full_arrive) {
                auto umma_arrive = [](const uint64_t* barrier) {
                    if constexpr (kNumMulticast == 1) {
                        cutlass::arch::umma_arrive(barrier);
                    } else {
            ...
            with_sf_b_barrier[stage_idx]->wait(phase);
            tcgen05_after_thread_sync();
            ...
            // Wait tensor memory empty barrier arrival
            //Todo: figure out the accumulator result
            tmem_empty_barriers[phase]->wait(accum_phase_idx ^ 1);
            tcgen05_after_thread_sync();

            // Launch MMAs
            for (int block_m_iter = scheduler.m_start; block_m_iter < scheduler.m_end; ++block_m_iter) {
                ...
                empty_barrier_arrive(true);
            }
            empty_barrier_arrive_b(true);
        }
        ...
        for (int block_k_iter = 0; block_k_iter < block_k; ++block_k_iter) {
            auto accum_stage_idx = scheduler.current_iter % kNumEpilogueStages;
            auto accum_phase_idx = (scheduler.current_iter / kNumEpilogueStages) & 1;
            int m_idx = scheduler.m_start;
            for (; m_idx < scheduler.m_end; ++m_idx) {
                // Wait UMMA arrival
