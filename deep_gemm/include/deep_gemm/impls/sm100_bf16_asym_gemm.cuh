...
sm100_bf16_asym_gemm_impl(uint32_t* offsets, uint32_t* experts,
    ...
    auto tensor_core_full_barrier   = barrier_start_ptr + kNumStages * 3 + kNumEpilogueStages * 2 + 2;
    ...
    uint32_t stage_idx = 0, phase = 0, tensor_core_phase = 0, phase_b = 0, phase_c = 0;
    ...
    if (warp_idx == 0 and cute::elect_one_sync()) {
        ...
        for (int block_k_iter = 0; block_k_iter < block_k; ++block_k_iter) {
            ...
            if constexpr (kMajorB == cute::UMMA::Major::K)
                printf("Kernel does not work in cute::UMMA::Major::K now");
            if constexpr (kMajorB == cute::UMMA::Major::MN)
                tma_copy<LOAD_BLOCK_N, BLOCK_K, kSwizzleBMode, cutlass::bfloat16_t, kIsBatchedMM>(
                        &tensor_map_b, full_barriers_b[0], smem_b[stage_idx], n_idx, k_idx, kNumMulticast, batch_idx);
            if (is_leader_cta) {
                full_barriers_b[0]->arrive_and_expect_tx(SMEM_B_SIZE_PER_STAGE * kNumMulticast);
            } else {
        ...
        for (int block_k_iter = 0; block_k_iter < block_k; ++block_k_iter) {
            ...
            for (uint32_t block_m_iter = scheduler.m_start; block_m_iter < scheduler.m_end; advance_pipeline(block_m_iter)) {
                ...
                tmem_empty_barriers[stage_idx]->wait(phase_c);
                phase_c ^= 1;
                tcgen05_after_thread_sync();
                    ...
                    for (uint32_t k = 0; k < BLOCK_K / UMMA_K; ++ k) {
                        ...
                        for (uint32_t w = 0; w < kNumMWaves; ++ w) {
                            ...
                            mma_t::fma(a_desc, b_desc,
                                       stage_idx * kNumMWaves * BLOCK_N + w * BLOCK_N,
                                       block_m_iter > 0 or k > 0,
                                       runtime_instr_desc);
                        }
                    }
                }
        ...
        if (kNumMulticast > 1 and iter_idx >= 0) {
            tmem_empty_barriers[iter_idx]->wait(phase_c);
        }
        ...
        for (int block_k_iter = 0; block_k_iter < block_k; ++block_k_iter) {
            for (int block_m_iter = scheduler.m_start; block_m_iter < scheduler.m_end; block_m_iter++) {
                ...
                // Wait UMMA arrival
                tmem_full_barriers[stage_idx]->wait(phase_c);
                tcgen05_after_thread_sync();
                ...
                for (uint32_t w = 0; w < kNumMWaves; ++ w) {
                    ...
                    for (uint32_t s = 0; s < kNumStores; ++ s, advance_store_pipeline()) {
                        ...
                        // The pipeline stage
                        const auto m_idx = block_m_iter * BLOCK_M;
                        ...
                        for (uint32_t i = 0; i < STORE_BLOCK_N / kNumElemsPerBankGroup; ++ i) {
                            ...
                            uint32_t tmem_addr = stage_idx * kNumMWaves * BLOCK_N +               // Accumulator offset
