...
const root=process.argv[2],out=path.join(root,'native-screenshots');fs.mkdirSync(out,{recursive:true});
...
await t.selection.selectSqlEvent('slice',Number(row.id),{scrollToSelection:false});
...
const actual=await p.evaluate(()=>({start:app.trace.timeline.visibleWindow.start.toTime().toString(),end:app.trace.timeline.visibleWindow.end.toTime().toString()}));if(actual.start!==evidence.viewport_start_ns||actual.end!==evidence.viewport_end_ns)throw Error('Viewport moved '+JSON.stringify(actual));await p.screenshot({path:path.join(out,key+'.png')});
