...
static void load_deep_gemm_so(const char* so_path) {
...
int main(int argc, char** argv) {
    // 1) Load deep_gemm._C*.so (set env var DEEP_GEMM_SO to the exact path)
    const char* so_path = std::getenv("DEEP_GEMM_SO");
    if (!so_path) {
        std::cerr
            << "Please set DEEP_GEMM_SO to the path of deep_gemm/_C*.so\n"
            << "Example:\n"
            << "  export DEEP_GEMM_SO=/path/to/site-packages/deep_gemm/_C.cpython-*.so\n";
        return 1;
    }
    load_deep_gemm_so(so_path);

    // 2) Dump op names so you can find the exact dispatcher schema name
    dump_ops_containing("deep_gemm");
    dump_ops_containing("m_grouped");

    // 3) TODO: set this to the REAL op name printed above
    // Common patterns look like:
    //   "deep_gemm::m_grouped_fp8_asym_gemm_nt_contiguous"
    // or "deep_gemm::m_grouped_fp8_asym_gemm_contiguous_1d1d"
    const std::string op_name =
        (argc >= 2) ? argv[1] : "deep_gemm::m_grouped_fp8_asym_gemm_nt_contiguous";
