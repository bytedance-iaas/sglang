...
def tag_updated_date(tag: DockerTag, timezone: ZoneInfo) -> date:
    ...
    if updated.tzinfo is None:
        raise SystemExit(f"Docker Hub timestamp lacks timezone: {tag.last_updated}")
...
def normalize_image_name(value: str, field_name: str) -> str:
    ...
    if "." in parts[0] or ":" in parts[0] or parts[0] == "localhost":
        if not HOST_RE.fullmatch(parts[0]) or ".." in parts[0]:
            raise SystemExit(f"invalid {field_name} registry host: {parts[0]}")
        start = 1
    if not parts[start:] or any(
        part in {"", ".", ".."} or not REPOSITORY_COMPONENT_RE.fullmatch(part)
...
def write_plan(path: str, plan: Iterable[SyncItem]) -> None:
    with open(path, "w", encoding="utf-8") as output:
        json.dump([item.__dict__ for item in plan], output, indent=2, sort_keys=True)
        output.write("\n")


def read_plan(path: str) -> list[SyncItem]:
    with open(path, encoding="utf-8") as source:
        payload = json.load(source)
    if not isinstance(payload, list):
        raise SystemExit("sync plan must be a JSON list")
    plan = []
    for entry in payload:
        if not isinstance(entry, dict) or set(entry) != {"source", "destination"}:
            raise SystemExit("invalid sync plan entry")
        plan.append(SyncItem(**entry))
    return plan
