...
cute::UMMA::SmemDescriptor make_umma_desc(dtype_t* base_smem_ptr, uint32_t mn_idx, uint32_t k_idx) {
    ...
    if constexpr (kMajorMode == cute::UMMA::Major::K) {
        // NOTES: for K-major layout, the swizzle must be 128B; allow multiple K-atoms if BLOCK_K is larger
        constexpr uint32_t kSwizzleBytes = 128;
        // DG_STATIC_ASSERT(kSwizzleMode == kSwizzleBytes, "K-major UMMA requires 128B swizzle");
        DG_STATIC_ASSERT((BLOCK_K * sizeof(dtype_t)) % kSwizzleBytes == 0, "BLOCK_K must be a multiple of 128B swizzle");

        // Atom size: 8 x `kSwizzleMode` (in bytes, on K)
        // {SBO, LBO} means the byte stride between atoms on {MN, K}
        // NOTES: on K, LBO is the per-atom stride; can be 0 when there is only 1 atom
        const uint32_t stride_byte_offset = num_non_contiguous * BLOCK_K * sizeof(dtype_t);
        const uint32_t leading_byte_offset = (BLOCK_K * sizeof(dtype_t) > kSwizzleBytes) ? kSwizzleBytes : 0;
