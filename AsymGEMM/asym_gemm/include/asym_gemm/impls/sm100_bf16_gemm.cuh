...
#include <asym_gemm/common/scheduler.cuh>
    ...
    if (warp_idx == 0 and cute::elect_one_sync()) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // if (lane_idx == 0 and m_block_idx == 0 and n_block_idx == 0 and k_block_idx == 0 and blockIdx.y == 0) {
                //     printf("DBG_BF16_GEMM LOAD warp=%u lane=%u stage=%u phase=%u m_idx=%u n_idx=%u k_a=%u k_b=%u\n",
                //            (unsigned)warp_idx, (unsigned)lane_idx, (unsigned)stage_idx, (unsigned)phase,
                //            (unsigned)m_idx, (unsigned)n_idx, (unsigned)k_a_idx, (unsigned)k_b_idx);
                // }
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // if (lane_idx == 0 and m_block_idx == 0 and n_block_idx == 0) {
                //     printf("DBG_BF16_GEMM MMA_LOAD stage=%u, BLOCK_K/UMMA_K=%u, A0..3=%f,%f,%f,%f B0..3=%f,%f,%f,%f\n",
                ...
                //            bf16_to_float(smem_b[stage_idx][0]), bf16_to_float(smem_b[stage_idx][1]),
                //            bf16_to_float(smem_b[stage_idx][2]), bf16_to_float(smem_b[stage_idx][3]));
                //     for (int i = 0; i < 64; i += 8) {
                //         printf("DBG_BF16_GEMM MMA_LOAD64 stage=%u off=%d "
                //                "A=%f,%f,%f,%f,%f,%f,%f,%f "
                //                "B=%f,%f,%f,%f,%f,%f,%f,%f\n",
                ...
                //                bf16_to_float(smem_b[stage_idx][i + 0]), bf16_to_float(smem_b[stage_idx][i + 1]),
                //                bf16_to_float(smem_b[stage_idx][i + 2]), bf16_to_float(smem_b[stage_idx][i + 3]),
                //                bf16_to_float(smem_b[stage_idx][i + 4]), bf16_to_float(smem_b[stage_idx][i + 5]),
                //                bf16_to_float(smem_b[stage_idx][i + 6]), bf16_to_float(smem_b[stage_idx][i + 7]));
                //     }
                // }
                    ...
                    // for (uint32_t k = 0; k < 1; ++ k) {
                        ...
                        // for (uint32_t w = 0; w < 1; ++ w) {
                            ...
                            mma_t::fma(a_desc, b_desc,
                                       ...
                                       k_block_idx > 0 or k > 0, // k_block_idx > 0 or k > 0
                                       runtime_instr_desc);
                        }
                    }
                }
                ...
                // if (m_block_idx == 0 and n_block_idx == 0) {
                ...
                //     printf("DBG_BF16_GEMM MMA_OUT threadIDx=%u k_block_idx=%u accum_stage=%u TMEM0..3=%f,%f,%f,%f\n",
                //            (unsigned)threadIdx.x, (unsigned)k_block_idx, (unsigned)accum_stage_idx,
                ...
                for (uint32_t s = 0; s < kNumStores; ++ s, advance_store_pipeline()) {
                    ...
                    for (uint32_t i = 0; i < STORE_BLOCK_N / kNumElemsPerBankGroup; ++ i) {
                        ...
                        if constexpr (cute::is_same_v<cd_dtype_t, float>) {
                            ...
                            cutlass::arch::fence_view_async_tmem_load();
                            // if (lane_idx == 0 and m_block_idx == 0 and n_block_idx == 0 and w == 0 and s == 0 and i == 0 and blockIdx.y == 0) {
                            //     printf("DBG_BF16_GEMM EPI_TMEM warp=%u epi_warp=%u lane=%u stage=%u tmem_addr=%u values=%f,%f,%f,%f\n",
