...
- `asym_gemm.set_num_sms`: set the maximum SM count to use
- `asym_gemm.get_num_sms`: get the current SM maximum count (return the device SM count if not set)
- `asym_gemm.set_tc_util`: set an approximated tensor core utilization ratio
- `asym_gemm.get_tc_util`: get the current tensor core utilization ratio
- `asym_gemm.transform_sf_into_required_layout`: transform scaling factors into required layout
- `asym_gemm.get_tma_aligned_size`: get the required TMA alignment size
- `asym_gemm.get_mk_alignment_for_contiguous_layout`: get the group-level alignment requirement for grouped contiguous layout
- `asym_gemm.get_mn_major_tma_aligned_tensor`: get a MN-major TMA-aligned tensor
- `asym_gemm.get_mn_major_tma_aligned_packed_ue8m0_tensor`: get a MN-major TMA-aligned tensor (with packing FP32 into UE8M0)
- `asym_gemm.get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor`: K-grouped GEMM packing kernel
...
- JIT cache related
  - `DG_JIT_CACHE_DIR`: string, the cache directory to store compiled kernels, `$HOME/.asym_gemm` by default
