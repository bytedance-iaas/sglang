...
def get_package_version():
    with open(Path(current_dir) / 'asym_gemm' / '__init__.py', 'r') as f:
...
def get_wheel_url():
    ...
    asym_gemm_version = get_package_version()
    ...
    wheel_filename = f'asym_gemm-{asym_gemm_version}+cu{cuda_version}-torch{torch_version}-cxx11abi{cxx11_abi}-{python_version}-{platform_name}.whl'
    wheel_url = base_wheel_url.format(tag_name=f'v{asym_gemm_version}', wheel_name=wheel_filename)
...
def get_ext_modules():
    ...
    return [CUDAExtension(name='asym_gemm._C',
                          sources=sources,
...
class CustomBuildPy(build_py):
    ...
    def generate_default_envs(self):
        ...
        with open(os.path.join(self.build_lib, 'asym_gemm', 'envs.py'), 'w') as f:
            f.write(code)
