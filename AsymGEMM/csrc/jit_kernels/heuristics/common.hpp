...
static SharedMemoryConfig get_smem_config(const GemmType& gemm_type, const KernelType& kernel_type,
    ...
    // const int& swizzle_a_mode = is_asym ? 16 : get_swizzle_mode(major_a == cute::UMMA::Major::K ? block_k : load_block_m, ab_elem_size);
    // const int& swizzle_b_mode = is_asym ? 16 : get_swizzle_mode(major_b == cute::UMMA::Major::K ? block_k : load_block_n, ab_elem_size);
    // const int& swizzle_cd_mode = ArchSpec::enable_cd_swizzle(cd_dtype) ? (is_asym ? 16 : get_swizzle_mode(block_n, cd_elem_size)) : 0;
    ...
    const int smem_b_total = is_asym ? smem_b_per_stage : num_stages * smem_b_per_stage;
    ...
    const int& smem_barrier = ArchSpec::get_barrier_smem_size(num_stages, is_asym);
...
static GemmConfig get_best_config(const GemmType& gemm_type, const KernelType& kernel_type,
    ...
    int block_k = 128 / static_cast<int>(c10::elementSize(ab_dtype));
...
template <typename ArchSpec>
static GemmConfig get_manual_config(const GemmType& gemm_type, const KernelType& kernel_type,
                                    const int& m, const int& n, const int& k, const int& num_groups,
                                    const cute::UMMA::Major& major_a, const cute::UMMA::Major& major_b,
                                    const at::ScalarType& ab_dtype, const at::ScalarType& cd_dtype,
                                    const bool& with_accumulation, const int& num_sms,
    ...
    DG_HOST_ASSERT(ArchSpec::is_block_size_legal(kernel_type, major_a, major_b,
                                                 ab_dtype, cd_dtype, m, n, k,
                                                 block_m, block_n, block_k));

    // Decide the number of TMA multicasts and whether broadcast on A
    MulticastConfig best_multicast_config = {1, false};
    const auto& [is_legal_on_a, is_legal_on_b] = ArchSpec::get_multicast_legality(
        gemm_type, num_groups, m, n, block_m, block_n, num_sms);
    const bool is_legal[2] = {is_legal_on_b, is_legal_on_a};
    bool order[2] = {false, true};
    if (block_m > block_n)
        std::swap(order[0], order[1]);
    ...
    if (num_stages > 0) {
        ...
        best_smem_config = get_smem_config<ArchSpec>(gemm_type, kernel_type,
                                                     ...
                                                     num_stages, best_multicast_config,
                                                     false);
        DG_HOST_ASSERT(best_smem_config.smem_size <= smem_capacity);
    } else {
        for (int stages = 32; stages > 0; --stages) {
            if (not ArchSpec::is_num_stages_legal(ab_dtype, cd_dtype, stages, block_m, block_n, block_k))
                continue;
            best_smem_config = get_smem_config<ArchSpec>(gemm_type, kernel_type,
                                                         m, n, k,
                                                         block_m, block_n, block_k,
                                                         major_a, major_b,
                                                         ab_dtype, cd_dtype,
                                                         stages, best_multicast_config,
                                                         false);
            if (best_smem_config.smem_size <= smem_capacity) {
                best_num_stages = stages;
                break;
            }
        }
        DG_HOST_ASSERT(best_num_stages != 0);
    ...
    const auto& get_num_waves = [=](const int& bm, const int& bn) {
        return ceil_div(get_num_blocks(bm, bn), num_sms);
    };
    ...
    const auto& config = GemmConfig {
        ...
        // ReSharper disable once CppLocalVariableMightNotBeInitialized
        .smem_config = best_smem_config,
        .thread_config = ArchSpec::get_thread_config(kernel_type, block_m, block_n)
    };
...
template <typename ArchSpec>
static GemmConfig get_best_config_asym(const GemmType& gemm_type, const KernelType& kernel_type,
                                  const int& m, const int& n, const int& k, const int& num_groups,
                                  const cute::UMMA::Major& major_a, const cute::UMMA::Major& major_b,
                                  const at::ScalarType& ab_dtype, const at::ScalarType& cd_dtype,
                                  const bool& with_accumulation, const int& num_sms) {
    ...
    constexpr int fixed_num_stages = 2;
    ...
    const int env_block_n = get_env<int>("DG_BLOCK_N", 0);
    const int env_block_k = get_env<int>("DG_BLOCK_K", 0);
    if (env_block_m > 0)
        block_ms = std::vector{env_block_m};
    if (env_block_n > 0)
        block_ns = std::vector{env_block_n};

    // Pick the smallest block M/N that allows the largest block K under fixed stages.
    constexpr int smem_capacity = ArchSpec::smem_capacity;
    int best_block_m = 0;
    int best_block_n = 0;
    int best_block_k = 0;
    ...
    const int max_block_k_by_swizzle = is_asym
        ...
        : ((major_a == cute::UMMA::Major::K or major_b == cute::UMMA::Major::K)
            ? 128 / ab_elem_size
            : 512 / ab_elem_size);
    const int max_block_k_cap = std::min(512 / ab_elem_size, max_block_k_by_swizzle);
    int max_block_k = k < max_block_k_cap ? k : max_block_k_cap;
    max_block_k = (max_block_k / block_k_step) * block_k_step;
    if (max_block_k < block_k_step)
        max_block_k = block_k_step;
    int min_block_k = block_k_step;
    if (env_block_k > 0) {
        DG_HOST_ASSERT(env_block_k >= block_k_step);
        DG_HOST_ASSERT(env_block_k <= max_block_k_cap);
        DG_HOST_ASSERT(env_block_k % block_k_step == 0);
        max_block_k = env_block_k;
        min_block_k = env_block_k;
    }
    ...
    for (const auto& block_m: block_ms) {
        for (const auto& block_n: block_ns) {
            ...
            for (const bool& is_multicast_on_a: order) {
                if (m >= 512 and is_legal[static_cast<int>(is_multicast_on_a)]) {
                    multicast_config = {2, is_multicast_on_a};
                    break;
                }
            }

            int candidate_block_k = 0;
            SharedMemoryConfig candidate_smem_config;
            for (int block_k = max_block_k; block_k >= min_block_k; block_k -= block_k_step) {
                if (not ArchSpec::is_block_size_legal(kernel_type, major_a, major_b, ab_dtype, cd_dtype,
                                                      m, n, k, block_m, block_n, block_k))
                    continue;
                if (not ArchSpec::is_num_stages_legal(ab_dtype, cd_dtype, fixed_num_stages, block_m, block_n, block_k))
                    continue;

                candidate_smem_config = get_smem_config<ArchSpec>(gemm_type, kernel_type,
                                                                  m, n, k,
                                                                  block_m, block_n, block_k,
                                                                  major_a, major_b,
                                                                  ab_dtype, cd_dtype,
            ...
            if (success) {
                ...
                best_block_k = candidate_block_k;
                best_multicast_config = multicast_config;
                best_smem_config = candidate_smem_config;
            }
        }
    }
    ...
    const auto& config = GemmConfig {
        ...
        .block_m = best_block_m,
        .block_n = best_block_n,
        .block_k = best_block_k,
        .num_stages = fixed_num_stages,
        .num_last_stages = ceil_div(k, best_block_k) % fixed_num_stages,
        .num_sms = num_min_sms,
        .tc_util = device_runtime->get_tc_util(),
        .multicast_config = best_multicast_config,
        // ReSharper disable once CppLocalVariableMightNotBeInitialized
...
template <typename ArchSpec>
static GemmConfig get_manual_config_asym(const GemmType& gemm_type, const KernelType& kernel_type,
                                  const int& m, const int& n, const int& k, const int& num_groups,
                                  const cute::UMMA::Major& major_a, const cute::UMMA::Major& major_b,
                                  const at::ScalarType& ab_dtype, const at::ScalarType& cd_dtype,
                                  const bool& with_accumulation, const int& num_sms,
    ...
    constexpr int fixed_num_stages = 2;
    if (num_stages > 0)
        DG_HOST_ASSERT(num_stages == fixed_num_stages);

    DG_HOST_ASSERT(ArchSpec::is_block_size_legal(kernel_type, major_a, major_b,
                                                 ab_dtype, cd_dtype, m, n, k,
                                                 block_m, block_n, block_k));
    DG_HOST_ASSERT(ArchSpec::is_num_stages_legal(ab_dtype, cd_dtype, fixed_num_stages,
                                                 block_m, block_n, block_k));
    ...
    constexpr int smem_capacity = ArchSpec::smem_capacity;
    const auto& smem_config = get_smem_config<ArchSpec>(gemm_type, kernel_type,
                                                        m, n, k,
                                                        block_m, block_n, block_k,
                                                        major_a, major_b,
                                                        ab_dtype, cd_dtype,
    ...
    const auto& config = GemmConfig {
        ...
        .block_k = block_k,
        .num_stages = fixed_num_stages,
        .num_last_stages = ceil_div(k, block_k) % fixed_num_stages,
        .num_sms = num_min_sms,
        .tc_util = device_runtime->get_tc_util(),
