...
class MetadataBuffers:
    def __init__(self, size: int, max_top_logprobs_num: int = 128,hidden_size: int=7168):
        # TODO: abort top_logprobs_num > 128 in PD

        # We transfer the metadata of first output token to decode
        # The minimal size for RDMA is 64Bytes, so we pad it to > 64Bytes
        self.output_ids = torch.zeros((size, 16), dtype=torch.int32, device="cpu")
        self.output_hidden_states = torch.zeros((size, hidden_size), dtype=torch.bfloat16, device="cpu")
