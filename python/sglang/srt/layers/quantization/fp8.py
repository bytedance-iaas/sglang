...
class Fp8MoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading_block_quant(self, layer: Module) -> None:
        ...
        if _is_fp8_fnuz:
            ...
            # Opt-in unified CPU(AMX INT8)+GPU(SM90 INT8) path. For an FP8
            # checkpoint this only engages when a pre-quantized INT8 dir is
            # provided (SGLANG_ASYMGEMM_UNIFIED_INT8_PATH); otherwise it is a
            # no-op and the FP8 asym_gemm path above stays in effect.
    ...
    def apply(
        ...
        # Unified asym_gemm INT8 fast path (when this layer was converted).
