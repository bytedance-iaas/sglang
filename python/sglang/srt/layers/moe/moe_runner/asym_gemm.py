...
def build_offsets_experts_from_masked_m(
    ...
    """
    active_experts_mask = masked_m > 0
    active_expert_indices = torch.where(active_experts_mask)[0].to(torch.int32)
    if active_expert_indices.numel() == 0:
        offsets = torch.tensor([], dtype=torch.int32, device=masked_m.device)
        experts = torch.tensor([-1], dtype=torch.int32, device=masked_m.device)
        return offsets, experts, 1
    v = masked_m[active_expert_indices]
    start = active_expert_indices * max_m
    end = start + ((v + block_m - 1) // block_m) * block_m
    # Interleave start and end
    offsets = torch.stack((start, end), dim=1).flatten()
    # Append -1 to experts
    experts = torch.cat((active_expert_indices.to(torch.int32), torch.tensor([-1], device=masked_m.device, dtype=torch.int32)))
    return offsets, experts, len(experts)
