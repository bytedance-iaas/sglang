...
def should_use_mega_moe(moe: DeepseekV2MoE, hidden_states: torch.Tensor) -> bool:
    ...
    if _device_sm == 90:
        if getattr(moe.experts, "_mega_moe_sm90_fp4_weights", False):
            if not is_sm90_fp4_mega_moe_available(moe.experts):
                raise RuntimeError(
                    "SM90 FP4 MegaMoE weights were built, but the required "
                    "DeepGEMM FP4 runtime is unavailable."
                )
        elif not is_sm90_fp8_mega_moe_available(moe.experts):
            return False
