...
class FusedMoE(torch.nn.Module):
    ...
    def __init__(
        ...
        self.use_asym_comp = get_moe_runner_backend().is_asym_comp()
        ...
        if kt_config is not None:
            if quant_config is not None:
                ...
                gpu_method = UnquantizedFusedMoEMethod(self.use_asym_comp)
            ...
            if self.quant_method is None:
                self.quant_method = UnquantizedFusedMoEMethod(
                    self.use_asym_comp, self.use_flashinfer_trtllm_moe
                )
    ...
    def _load_w13(
        ...
        if _is_cpu:
            ...
            if not self.use_presharded_weights:
                if not is_bias and self.use_asym_comp:
