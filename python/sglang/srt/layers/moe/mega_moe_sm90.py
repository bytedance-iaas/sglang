...
def _transpose_scale_for_utccp(scale: torch.Tensor) -> torch.Tensor:
    ...
    if mn % 128 != 0:
        raise ValueError(
            f"SM90 MegaMoE scale M dimension must be 128-aligned, got {mn}"
        )
    return (
...
def build_sm90_mega_moe_experts_weights(experts) -> None:
    ...
    if w13.dtype != torch.float8_e4m3fn or w2.dtype != torch.float8_e4m3fn:
        raise TypeError(
            f"SM90 FP8 MegaMoE weights must be float8_e4m3fn, got {w13.dtype}/{w2.dtype}"
        )
    ...
    expected = (
        (expected_n_groups_1, expected_k_groups_1),
        (expected_n_groups_2, expected_k_groups_2),
    )
    actual = (
        (w13_sf_fp32.shape[1], w13_sf_fp32.shape[2]),
        (w2_sf_fp32.shape[1], w2_sf_fp32.shape[2]),
    )
    if actual != expected:
        raise ValueError(
            f"SM90 FP8 scale groups mismatch: got {actual}, expected {expected}"
        )
...
def build_sm90_fp4_mega_moe_experts_weights(experts) -> None:
    ...
    if w13.dtype not in (torch.int8, torch.uint8) or w2.dtype not in (
        torch.int8,
        torch.uint8,
    ):
        raise TypeError(
            f"SM90 FP4 MegaMoE packed weights must be int8/uint8, got {w13.dtype}/{w2.dtype}"
        )
    if w13_sf_fp32.dtype != torch.float32 or w2_sf_fp32.dtype != torch.float32:
        raise TypeError(
            f"SM90 FP4 MegaMoE scales must be float32 before transform, got {w13_sf_fp32.dtype}/{w2_sf_fp32.dtype}"
        )
