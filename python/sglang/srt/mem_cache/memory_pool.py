...
class KVBit4MLATokenToKVPool(DSATokenToKVPool):
    ...
    (``index_k_with_scale_buffer``, 128-dim, used for top-k routing) is
    allocated and served UNCOMPRESSED — the indexer must stay exact; kvbit
    only compresses the MLA attention latent. Only the MLA KV buffer
    (``kv_buffer``) is overridden: 4bit packed rotated nope + raw BF16 rope,
    no full BF16 kv_buffer (no_alloc → ~2.77x token capacity: 416 B/token vs
    1152 B/token native).
    ...
    kernel reads ``kvbit_packed`` (4bit rotated nope) + ``rope_buffer`` (bf16
    rope) on-the-fly. ``get_key_buffer`` materializes the full BF16 pool
    (4bit decode + concat rope) for the prefill/fa3 fallback only. With Q-FHT,
    the Hadamard rotation folds to the query side (``q_nope_absorbed @ R``) so
    the decode kernel reads the rotated latent directly; prefill keeps
    unfolded q + unrotated K (get_key_buffer inverse-rotates).
    """
    ...
    def __init__(
        ...
        # Defer to DSATokenToKVPool: it allocates the indexer K cache
        # (index_k_with_scale_buffer) and calls MLATokenToKVPool.__init__ which
        # calls _create_buffers (our override below builds the 4bit store
        # instead of the bf16 kv_buffer). kv_cache_dim passed as the DSA
        # kv_cache_dim arg.
        kv_cache_dim = (
            override_kv_cache_dim
