...
class KVBit4MLATokenToKVPool(MLATokenToKVPool):
    ...
    This is the sglang-native KV store for kvbit's no_alloc route (built in
    sglang, not monkey-patched). The decode path bypasses fa3 entirely: a
    kvbit-owned triton paged decode kernel reads ``kvbit_packed`` (4bit rotated
    nope) + ``rope_buffer`` (bf16 rope) on-the-fly, so the full-size BF16
    ``kv_buffer`` is never needed and is NOT allocated (no_alloc → real HBM
    win, ~2.77x token capacity: 416 B/token vs 1152 B/token native).
    ...
    ``set_mla_kv_buffer`` encodes the nope latent via kvbit (lazy-imported) and
    scatters into ``kvbit_packed``; rope is copied raw into ``rope_buffer``.
    ``get_key_buffer`` materializes the full BF16 pool (4bit decode + concat
    rope) — used only by the prefill/fa3 fallback path; the decode bypass
    kernel reads the packed stores directly and never calls this.

    kvbit's Hadamard rotation is valid here because the MLA nope latent
    (kv_lora_rank) is position-INdependent (unlike Qwen3.5's full-RoPE K, which
    has no nope block and is NOT a kvbit rotation target). With Q-FHT, the
    rotation folds to the query side (``q_nope_absorbed @ R``) so the decode
    kernel skips the per-K inverse-rotate and uses the rotated latent directly.
    """

    # kvbit 4bit config — fixed for the GLM-5.2 production layout.
    kvbit_bits: int = 4
    ...
    def _create_buffers(self):
        ...
        with self.memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            with (
                ...
                # Raw BF16 rope tail: (m, 1, qk_rope_head_dim). Kept native
                # (rope is position-dependent, not kvbit-compressed; 128 B/token
                # is 11% of native KV — keeping it native is fine, the win is
                # the nope 512 -> 4bit).
                self.rope_buffer = [
                    torch.zeros(
    ...
    def get_key_buffer(self, layer_id: int):
        ...
        reads ``kvbit_packed`` + ``rope_buffer`` directly and does NOT call
        this. Decoding the whole pool every call is O(pool_size) — acceptable
        for prefill (infrequent), NOT for decode (the bypass kernel avoids it).
        """
        if self.layer_transfer_counter is not None:
            self.layer_transfer_counter.wait_until(layer_id - self.start_layer)
