...
| `managers/scheduler.py` `__init__` | `self.enable_eic_cache` = flag AND tree cache is an EIC class | EIC bookkeeping must follow what was built, not the flag | #737 | none |
...
| Item | Reason |
|---|---|
| `scripts/eic_gate.py` (#760) | Duplicates `scripts/eic_integration_check.py`, which is already on ep_main |
