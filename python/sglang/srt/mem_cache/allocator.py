...
class DcpTokenToKVPoolAllocator(BaseTokenToKVPoolAllocator):
    ...
    def __init__(
        ...
        # NOTE: we deliberately do not call super().__init__ because it would
        # set ``free_pages`` / ``release_pages`` / ``is_not_in_free_group`` /
        # ``free_group`` as plain attributes on ``self`` — but we want those
        # to be @property forwards into ``self.real_allocator``.
        self.size = size  # logical, externally-visible
        self.page_size = page_size  # logical page size
        self.dtype = dtype
        self.device = device
        self._kvcache = kvcache
        self.need_sort = need_sort
    ...
    def filter_local_indices(self, indices: torch.Tensor) -> torch.Tensor:
        """Return the subset of ``indices`` that live on this rank, mapped
        to local (per-rank-physical) offsets.

        Useful for callers that want to operate on this rank's physical KV
        storage directly (e.g. data movement or debugging tools).
        """
