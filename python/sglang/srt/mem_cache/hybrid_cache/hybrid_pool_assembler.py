...
class _MambaStrategy(StackStrategy):
    ...
    def build(
        ...
        *,
        ...
        # Completion counters are stage-local; the pool maps use global layer ids.
        full_layer_mapping = {
            layer - kvcache.start_layer: index
            for layer, index in kvcache.full_attention_layer_id_mapping.items()
        }
        mamba_layer_mapping = {
            layer - kvcache.start_layer: index
            for layer, index in params.req_to_token_pool.mamba_map.items()
        }
