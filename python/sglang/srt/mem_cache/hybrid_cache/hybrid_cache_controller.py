...
class HybridCacheController(BaseHiCacheController):
    ...
    def _page_transfer(self, operation: PrefetchOperation) -> None:
        # KV and KV-derived sidecars determine the synchronized target prefix.
    ...
    def _page_transfer_sidecar(
        ...
        if not operation.is_terminated() and kv_completed_pages == len(
            ...
            pool_hits = count_pool_hits(
                self.storage_backend.batch_get_v2(transfers_nonkv)
            )

        # Emit this ACK even after cancellation so every rank executes the
        # same completion collective sequence.
    ...
    def _reduce_prefetch_ack(self, ack: PrefetchAck) -> None:
        ...
        for index, pool in enumerate(PoolName):
            ack.pool_hits[pool.value] = packed[index].item()
