...
class UnifiedRadixCache(KVCacheEventMixin, BasePrefixCache):
    ...
    def _insert_helper_host(
        ...
        # In non-write-back modes, a host-backed child under an un-backed-up
        # parent violates the write-through tree invariant. The parent can
        # otherwise be evicted as a device-only leaf and orphan the refilled
        # child. Match upstream by dropping the refill and falling back to
        # normal prefill instead of synchronously backing up the parent here.
        is_non_write_back = (
            self.cache_controller is not None
            and self.cache_controller.write_policy != "write_back"
        )
        if node is not self.root_node and not node.backuped and is_non_write_back:
            logger.info(
