...
class UnifiedRadixCache(BasePrefixCache):
    ...
    def init_hicache(self, server_args: ServerArgs, params: CacheInitParams) -> None:
        ...
        self._init_hicache_pp_sync_mode(server_args)

    def _init_hicache_pp_sync_mode(self, server_args: ServerArgs) -> None:
        mode = envs.SGLANG_HICACHE_PP_SYNC_MODE.get()
        if mode not in {"legacy", "layered_flags"}:
            raise ValueError(
