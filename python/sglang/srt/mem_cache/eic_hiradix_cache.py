...
class EICHiRadixCache(RadixCache):
    ...
    def insert(self, key: RadixKey, value=None, chunked=False, eic_backup=True):
        if self.disable:
            return 0
    ...
    def _insert_helper(self, node: TreeNode, key: RadixKey, value, eic_backup=True):
        node.last_access_time = time.monotonic()
        if len(key) == 0:
            return 0
    ...
    def match_from_remote(self, waiting_queue: List[Req]):
        ...
        for req in waiting_queue:
            ...
            if req.rid in self.match_req_set:
                continue
            req_tokens = req.adjust_max_prefix_ids()
