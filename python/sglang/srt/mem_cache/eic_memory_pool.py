...
class EICDeepSeekV4TokenToKVPoolHost(EICBaseTokenToKVPoolHost):
    ...
    def device_writeback(self, device_indices, src_tensor, src_indices):
        ...
        # load does not); mirror HiCacheController.move_indices for "kernel".
        if self.io_backend == "kernel":
            full_host_indices = full_host_indices.to(device_indices.device)
            transfers = [
                PoolTransfer(
                    name=t.name,
                    host_indices=t.host_indices.to(t.device_indices.device),
                    device_indices=t.device_indices,
                )
                for t in transfers
            ]
