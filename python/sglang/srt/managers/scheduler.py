    ...
    def init_cache_with_memory_pool(self):
        ...
        if effective_chunked_prefill_size is not None and self.disable_radix_cache:
            ...
            if envs.SGLANG_EXPERIMENTAL_CPP_RADIX_TREE.get():
                ...
                elif self.enable_eic_cache:
                    # EIC has native hierarchical integration; route through its
                    # builder so prefetch results land in the radix tree (otherwise
                    # HiRadix + EICStorage fetches from remote but never registers
                    # the prefix as a hit).
                    self.tree_cache = EICHiRadixCacheBuilder.build(
                        params=params,
                        server_args=server_args,
