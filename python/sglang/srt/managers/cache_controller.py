...
@dataclass
class PrefetchAck:
    """Synchronized progress or completion for one L3 prefetch."""
...
class HiCacheController:
    ...
    def __init__(
        ...
        # Storage control queues are recreated whenever storage threads start.
        ...
        self.ack_prefetch_queue: Queue[PrefetchAck] = Queue()
    ...
    def attach_storage_backend(
        ...
        try:
            ...
            # Hit queries and completion ACKs run on different threads, so use
            # distinct groups to keep their collective ordering independent.
    ...
    def _page_transfer(self, operation: PrefetchOperation) -> int:
        ...
        for i in range(0, len(operation.hash_value), STORAGE_BATCH_SIZE):
            # Keep emitting the same number of ACKs after a local failure or
            ...
            if all_success:
                ...
                # Draft KV must be present before target completion is published.
                if self.has_draft:
    ...
    def _page_transfer_kv_batch(
        ...
        if kv_derived_transfers:
            ...
            sidecar_hits = count_pool_hits(
                self.storage_backend.batch_get_v2(current_transfers)
            )
    ...
    def prefetch_sync_thread_func(self):
        """Synchronize prefetch progress across PP and attention ranks."""
        while not self.storage_stop_event.is_set():
            try:
