...
from sglang.srt.multimodal.mm_utils import fast_image_hash, image_to_int, generate_reconstruct_cudatensor_infos, operate_substrings, insert_input_ids
...
class Glm4vImageProcessor(SGLangBaseProcessor):
    ...
    def process_mm_data(
        ...
        if cache_mm_image_items and is_glm4v_processor:
            ...
            to_replace_str = self.IMAGE_START_TOKEN + self.IMAGE_TOKEN + self.IMAGE_END_TOKEN
            ...
            for img_idx in range(len(images)):
                ...
                if img_idx in new_processed_img_idxes:
                    ...
                    delete_key = self.image_cache_table.add(
                        img_hash_keys[img_idx],  {"pixel_values": to_cache_tensor, "target_shape" : (target_height, target_width)}
                    )
                    
                    tensor_lists.append(to_cache_tensor.contiguous())
                # add input ids and insert tensor
                else:
                    cached_tensor = self.image_cache_table.get(img_hash_keys[img_idx])["pixel_values"]
                    assert isinstance(
    ...
    async def process_mm_data_async(
        ...
        **kwargs,
        ...
        cache_mm_image_items = get_bool_env_var("SGL_CACHE_MM_IMAGE")
        is_glm4_5_v_processor = isinstance(self._processor.image_processor, transformers.models.glm4v.image_processing_glm4v_fast.Glm4vImageProcessorFast)
