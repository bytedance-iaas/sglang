...
# The logging/system-metrics instrumentors are auxiliary: they enrich traces
# with log correlation and host metrics but are not required for the gen-ai
# spans and token/latency histograms. Keep them optional so a serving image
# that ships only the core OpenTelemetry SDK still exports gen-ai observability
# instead of silently disabling everything.
_is_otel_instrumentation_available = False
try:
    ...
    _is_otel_instrumentation_available = True
except ImportError:
...
def init_tracer(instrumenting_module_name: str) -> Optional[Tracer]:
    ...
    if _is_otel_instrumentation_available:
