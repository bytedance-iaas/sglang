...
class EICConnector(BaseKVConnector):
    ...
    def memcpy_tensor(self, dst_tensor: torch.Tensor, src_bytes: bytes):
        ...
        if len(src_bytes) != dst_size:
            raise ValueError(
                f"Size mismatch: dst tensor size = {dst_size}, src bytes = {len(src_bytes)}"
            )
    ...
    def mget(
        ...
        if tensors is not None:
            ...
            for tensor in tensors:
                data_vals.append(
                    tensor.data_ptr(), tensor.element_size() * tensor.numel(), False
                )
        else:
            data_vals = None
        ...
        if status_code != eic.StatusCode.SUCCESS:
            logger.error(
                f"eic mget {keys} failed, status_code {status_code} PID = {os.getpid()}"
            )
        ...
        for i, key in enumerate(keys):
            success = (
                ...
                and get_outcome.status_codes[i] == eic.StatusCode.SUCCESS
            )
            ...
            if success:
                if tensors is None:
                    ...
                    tensor = torch.from_numpy(
                        np.frombuffer(byte_data, dtype=np.int8).copy()
                    )
                    result_tensors[i] = tensor
            else:
                logger.debug(
                    f"eic mget failed for key: {key}, fallback to object storage"
                )
        ...
        for i, future in object_futures:
            ...
            if content is None:
                logger.error(
                    f"Failed to get tensor from both EIC and object storage for key: {keys[i]}"
                )
                return None
            else:
                if tensors is None:
    ...
    def mset(self, input_keys: List[str], tensors: List[torch.Tensor]) -> None:
        ...
        for tensor in tensors:
            ...
            vals.append(
                cpu_tensor.data_ptr(),
                cpu_tensor.element_size() * cpu_tensor.numel(),
                False,
            )
        ...
        if status_code != eic.StatusCode.SUCCESS:
            ...
            for i, err_code in enumerate(set_outcome.status_codes):
                if err_code != eic.StatusCode.SUCCESS:
                    logger.error(f"eic mset {keys[i]} failed, err_code {err_code}")
    ...
    def _set_to_object_storage(self, key: str, tensor: torch.Tensor) -> None:
        ...
        try:
            self.object_storage_client.put_object(
                Bucket=self.bucket_name, Key=key, Body=byte_data
            )
            logger.info(f"S3 put_object success, bucket={self.bucket_name}, key={key}")
        except ClientError as e:
            logger.error(
                f"S3 put_object failed, bucket={self.bucket_name}, key={key}, error={e}"
            )
        except Exception as e:
            logger.error(
                f"S3 put_object unexpected error, bucket={self.bucket_name}, key={key}, error={e}"
            )
    ...
    def _get_from_object_storage(self, key: str) -> (bytes, torch.Tensor):
        ...
        try:
            ...
            logger.error(
                f"S3 get_object failed, bucket={self.bucket_name}, key={key}, error={e}"
            )
            return None, None
        except Exception as e:
            logger.error(
                f"S3 get_object unexpected error, bucket={self.bucket_name}, key={key}, error={e}"
            )
            return None, None
    ...
    def _get_from_object_storage_async(self, key: str) -> concurrent.futures.Future:
        def _object_storage_get_task():
            ...
            try:
                ...
                tensor = torch.from_numpy(np_array)
                logger.debug(
                    f"S3 get_object success, bucket={self.bucket_name}, key={key}"
                )
                return content, tensor
            except ClientError as e:
                logger.error(
    ...
    def pull_files(
        ...
        logger.debug(
            f"start download model config from eic, key: {config_key}, path {self.local_dir}"
        )
        keys = [config_key]
        tensors = self.mget(keys)
        if tensors is None:
            logger.error(f"download model config from eic failed, key: {config_key}")
            return False
        ...
        logger.debug(
            f"download finished, model config from eic, key: {config_key}, path {self.local_dir}"
        )
        ...
        for i, tensor in enumerate(data_tensors):
            ...
            if (
                os.path.exists(local_path_file)
                and os.path.getsize(local_path_file)
                == tensor.numel() * tensor.element_size()
            ):
                logger.info(f"model config file {local_path_file} already exists, skip")
                continue
        ...
        config_file = os.path.join(self.local_dir, EICConnector.PULL_CONFIG_FILE)
    ...
    def upload_model_config(self, model_name: str, file_list: List[str]):
        ...
        config_json = {"file_num": file_num, "file_name_list": base_name_list}
        config_json_bytes = json.dumps(config_json).encode()
        config_buffer = torch.from_numpy(
            np.frombuffer(config_json_bytes, dtype=np.int8).copy()
        )
    ...
    def save_model(self, model_prefix, model_name, local_model_path, rank, state_dict):
        ...
        for key, tensor in state_dict.items():
            ...
            if should_write_data and meta_size >= self.MODEL_META_APPROXIMATE_SIZE:
                ...
                meta_buffer = torch.from_numpy(
                    np.frombuffer(meta_bytes, dtype=np.int8).copy()
                )
        ...
        if meta_json["kv_num"] > 0:
            ...
            root_json["meta_num"] += 1
        # write root
        logger.info(f"write root {model_prefix}, content: {root_json}")
        root_bytes = json.dumps(root_json).encode()
        root_buffer = torch.from_numpy(np.frombuffer(root_bytes, dtype=np.int8).copy())
        ...
        logger.info(f"save model on rank{rank} to eic cost {end_time - start_time}s")
    ...
    def _read_huge_kv(
        ...
        for i in range(split_num):
            ...
            self.perf_statistics["read_huge_kv_split_s"] = (
                self.perf_statistics.get("read_huge_kv_split_s", 0)
                + split_end_time
                - split_start_time
            )
            self.perf_statistics["read_huge_kv_split_ops"] = (
                self.perf_statistics.get("read_huge_kv_split_ops", 0) + 1
            )
            param_data.view(torch.int8).view(value_length)[start:end].copy_(
                self.split_tensors[thread_index][0 : end - start]
            )
            load_end_time = time.perf_counter()
            self.perf_statistics["read_huge_kv_copy_split_s"] = (
                self.perf_statistics.get("read_huge_kv_copy_split_s", 0)
                + load_end_time
                - split_end_time
            )
        end_time = time.perf_counter()
        self.perf_statistics["read_huge_kv_s"] = (
            self.perf_statistics.get("read_huge_kv_s", 0) + end_time - start_time
        )
        self.perf_statistics["read_huge_kv_ops"] = (
            self.perf_statistics.get("read_huge_kv_ops", 0) + 1
        )
    ...
    def _read_kv(
        ...
        end_time = time.perf_counter()
        self.perf_statistics["read_multi_kv_s"] = (
            self.perf_statistics.get("read_multi_kv_s", 0) + end_time - start_time
        )
        self.perf_statistics["read_multi_kv_ops"] = (
            self.perf_statistics.get("read_multi_kv_ops", 0) + 1
        )
        return tensors
    ...
    def _read_meta(
        ...
        for i in range(self.meta_num):
            meta_name = os.path.join(model_name, self.meta_prefix, str(i))
    ...
    def _read_root(
        ...
        logger.info(f"read root {root_name}, content: {root_json}")
        ...
        if self.ENABLE_ZERO_COPY:
            ...
            for i in range(self.thread_num):
                zero_copy_ptr = self.allocate_zero_copy_buffer(self.data_split_size)
                ubyte_ptr = ctypes.cast(zero_copy_ptr, ctypes.POINTER(ctypes.c_ubyte))
                byte_array = (ctypes.c_ubyte * self.data_split_size).from_address(
            ...
            if not self.ENABLE_THREADING:
                self.thread_num = 1
            for i in range(self.thread_num):
                split_tensor = torch.empty(self.data_split_size, dtype=torch.int8)
                self.split_tensors.append(split_tensor)

    def _load_eic_worker(
        self, model_name, state_dict, sub_items: list[tuple[str, Any]], thread_index
    ):
        # iterate all keys
        batch_size = 0
        eic_keys = []
