...
class PipelineReadyQueueSchedule:
    def __init__(
        ...
        self._running: list[Optional[PipelineWavefrontAction]] = [None] * physical_size
    ...
    def actions(
        self,
        tick: int,
        is_ready: Optional[Callable[[int, int, int], bool]] = None,
    ) -> Tuple[Optional[PipelineWavefrontAction], ...]:
        if tick < 0:
            raise ValueError("ready-queue tick must be non-negative")
        actions = []
        for physical_rank, ready in enumerate(self._ready):
            if self._running[physical_rank] is not None or not ready:
                actions.append(None)
                continue
            available = [
                item
                for item in ready
                if is_ready is None or is_ready(physical_rank, item[0], item[1])
            ]
            if not available:
                actions.append(None)
                continue
            decode = [item for item in available if item[1] >= self.physical_size]
            candidates = decode if decode else available
            ...
            self._running[physical_rank] = action
    ...
    def complete(
        ...
        for physical_rank, action in enumerate(actions):
            ...
            if self._running[physical_rank] != action:
                raise RuntimeError(
                    f"VPP action {action} is not running on PP rank {physical_rank}"
                )
            self._running[physical_rank] = None
