...
from sglang.srt.speculative.dspark_utils import (
    can_dspark_use_fused_qkv_proj,
    parse_dspark_draft_config,
    resolve_dspark_verify_mask_policy,
)
...
class DSparkWorker:
    """DSpark speculative decoding worker (spec-v1, tp>=1/pp=1)."""
    ...
    def __init__(
        ...
        if draft_backend is None:
            ...
            logger.warning(
                "DSPARK draft worker does not support 'trtllm_mha' because the "
        ...
        if server_args.speculative_num_draft_tokens is None:
            ...
            self.block_size = int(server_args.speculative_num_draft_tokens) - 1
            ...
            if model_block_size is not None and int(model_block_size) != int(
                ...
                logger.warning(
                    "DSPARK block size mismatch: using speculative_num_draft_tokens=%s but draft config block_size=%s.",
                    self.block_size,
                    model_block_size,
                )
        self.proposal_len = int(self.block_size)
        self.verify_len = int(self.proposal_len + 1)
        self.confidence_threshold = float(
            getattr(server_args, "speculative_dspark_confidence_threshold", 0.0) or 0.0
        )
        ...
        if self.tp_rank == 0:
            logger.info(
                "Initialized DSPARK draft runner. attention_backend=%s, model=%s, proposal_len=%s, verify_len=%s, confidence_threshold=%s, draft_window_size=%s, compact_cache=%s",
            ...
            logger.info(
                "DSPARK draft runner ready. mask_token=%s, mask_token_id=%s, mask_token_id_override=%s",
                self._mask_token,
                self._mask_token_id,
        ...
        self._verify_block_positions_buf: Optional[torch.Tensor] = (
            None  # [cap_bs, verify_len]
        )
        ...
        self._draft_block_spec_info = DSparkVerifyInput(
            ...
            draft_token_num=int(self.proposal_len),
            custom_mask=None,
    ...
    def _init_fused_kv_helper(self) -> None:
        ...
        try:
            ...
            for layer_idx, layer in enumerate(layers):
                ...
                eligible, reason = can_dspark_use_fused_qkv_proj(attn.qkv_proj)
                if not eligible:
            ...
            if self.tp_rank == 0:
                logger.info(
                    "DSPARK fused KV materialization enabled. "
            ...
            logger.warning(
                "DSPARK fused KV initialization failed, falling back to sequential path: %s",
                e,
            )
            self._use_fused_kv_materialize = False
            self._fused_kv_helper = None
    ...
    def _ensure_draft_block_buffers(self, bs: int) -> None:
        ...
        self._verify_block_positions_buf = torch.empty(
            (new_cap, self.verify_len), dtype=torch.int64, device=device
        )
        self._draft_block_tokens_buf = torch.empty(
            (new_cap, self.verify_len), dtype=torch.long, device=device
        )
    ...
    def _gather_req_to_token_segments(
        ...
        *,
        ...
        return self._gather_req_to_token_masked(
            ...
            pos2d=pos2d,
            mask=mask,
            context="DSPARK req_to_token segment gather",
        )
    ...
    def _resolve_mask_token_id(
        ...
        if not isinstance(mask_token, str) or not mask_token:
            raise ValueError(
                f"DSPARK mask_token must be a non-empty string, got {mask_token!r}."
            )
        ...
        if mask_token_id is not None:
            ...
            if resolved_id >= vocab_size:
                raise ValueError(
                    "DSPARK mask_token_id is outside the target vocab size. "
                    ...
                    "SGLang does not support resizing target embeddings for DSPARK yet."
                )
            ...
            if tokenizer is not None:
                ...
                if (
                    ...
                    raise ValueError(
                        "DSPARK config mismatch: dspark_config.mask_token_id conflicts with tokenizer vocab id "
                        f"for dspark_config.mask_token. mask_token={mask_token!r}, "
        ...
        if tokenizer is None:
            raise RuntimeError(
                "DSPARK requires tokenizer initialization when dspark_config.mask_token_id is not set "
        ...
        if resolved_id is None:
            # Mirror the reference DSpark HF demo by adding the mask token to the tokenizer.
            ...
            if added and self.tp_rank == 0:
                logger.info(
                    "Added DSPARK mask token to tokenizer. token=%s, mask_token_id=%s, tokenizer_len=%s, model_vocab_size=%s",
                    mask_token,
                    resolved_id,
                    len(tokenizer),
                    vocab_size,
        ...
        if resolved_id is None or int(resolved_id) < 0:
            raise ValueError(
                "DSPARK requires resolving a mask token id, but it could not be resolved. "
    ...
    def _prepare_for_speculative_decoding(
        self, batch: ScheduleBatch, draft_input: DSparkDraftInput
    ):
        ...
        if batch.sampling_info is not None and not batch.sampling_info.is_all_greedy:
            raise RuntimeError(
                "DSPARK MVP currently supports greedy decoding only. "
                "Set temperature=0 or disable non-greedy sampling."
            )
        ...
        if (
            ...
            raise RuntimeError(
                "DSPARK requires the target model to expose a vocab-parallel `lm_head` with `weight` and "
        ...
        assert self._verify_block_positions_buf is not None
        ...
        draft_next, max_accept_lens = self._greedy_sample_dspark_block(
            draft_hidden=draft_hidden,
            first_prev_token_ids=block_ids[:, 0],
            lm_head=lm_head,
        )
        ...
        verify_positions_2d = self._verify_block_positions_buf[:bs]
        torch.add(
            target_prefix_lens.unsqueeze(1),
            self._verify_pos_offsets,
            out=verify_positions_2d,
        )
        ...
        verify_input = DSparkVerifyInput(
            ...
            positions=positions,
            draft_token_num=self.verify_len,
            max_accept_lens=max_accept_lens,
        )
        _, build_custom_mask = resolve_dspark_verify_mask_policy(
    ...
    def _greedy_sample_from_vocab_parallel_head(
        ...
        *,
        ...
        lm_head,
        local_bias: Optional[torch.Tensor] = None,
        chunk_size: int = 256,
    ) -> torch.Tensor:
        ...
        if not hasattr(lm_head, "weight") or not hasattr(lm_head, "shard_indices"):
            raise RuntimeError(
                "DSPARK greedy sampling requires a vocab-parallel head with `weight` and `shard_indices`."
            )
            ...
            for start in range(0, num_tokens, int(chunk_size)):
                ...
                if num_org > 0:
                    ...
                    if local_bias is not None:
                        base_logits = base_logits + local_bias[start:end, :num_org].to(
        ...
        for start in range(0, num_tokens, int(chunk_size)):
            ...
            if num_added > 0:
                ...
                if local_bias is not None:
                    added_logits = added_logits + local_bias[
                        start:end, added_slice_start:added_slice_end
                    ].to(dtype=added_logits.dtype)
    ...
    def _greedy_sample_dspark_block(
        self,
        *,
        ...
        """Greedy DSpark proposal with optional Markov correction and confidence cap."""
        bs, proposal_len, _ = draft_hidden.shape
        draft_model = self.draft_model
        has_markov = int(getattr(draft_model, "markov_rank", 0) or 0) > 0
        confidence_logits = []
        sampled_tokens = []
        prev_token_ids = first_prev_token_ids.to(torch.long)
        markov_state = None

        if not has_markov:
            flat_hidden = draft_hidden.reshape(-1, draft_hidden.shape[-1])
            logits_hidden = (
                draft_model.get_logits_hidden(flat_hidden)
                if hasattr(draft_model, "get_logits_hidden")
                else flat_hidden
            )
            sampled = self._greedy_sample_from_vocab_parallel_head(
                hidden_states=logits_hidden,
                lm_head=lm_head,
            ).view(bs, proposal_len)
            if getattr(draft_model, "confidence_head", None) is not None:
                prev_for_conf = torch.cat(
                    [first_prev_token_ids[:, None], sampled[:, :-1]], dim=1
                )
                conf = draft_model.predict_confidence(
                    hidden_states=draft_hidden.reshape(-1, draft_hidden.shape[-1]),
                    prev_token_ids=prev_for_conf.reshape(-1),
                )
                if conf is not None:
                    confidence_logits.append(conf.view(bs, proposal_len))
            return sampled, self._confidence_to_max_accept_lens(
                confidence_logits[0] if confidence_logits else None,
                bs=bs,
        ...
        for step in range(proposal_len):
            ...
            logits_hidden = (
                draft_model.get_logits_hidden(step_hidden)
                if hasattr(draft_model, "get_logits_hidden")
                else step_hidden
            )
            local_bias, markov_state = draft_model.compute_markov_bias_local(
            ...
            if conf is not None:
                confidence_logits.append(conf)
            next_token_ids = self._greedy_sample_from_vocab_parallel_head(
                hidden_states=logits_hidden,
                lm_head=lm_head,
        ...
        sampled = torch.stack(sampled_tokens, dim=1)
        confidence = (
    ...
    def _confidence_to_max_accept_lens(
        self,
        ...
        *,
        ...
        if (
            ...
            return torch.full(
                (bs,), int(proposal_len), dtype=torch.int32, device=device
            )
        probs = torch.sigmoid(confidence_logits.float())
        # Match DeepSpec eval: truncate at the first draft position whose local
        # confidence is below threshold.
        return (
            (probs >= self.confidence_threshold)
            .to(torch.int32)
            .cumprod(dim=1)
            .sum(dim=1)
        )
    ...
    def _append_target_hidden_to_draft_kv(
        ...
        if draft_input.target_hidden is None:
            raise RuntimeError(
                "DSPARK draft state missing target_hidden context features."
            )
        ...
        with torch.inference_mode():
            ...
            if self._use_fused_kv_materialize and self._fused_kv_helper is not None:
                try:
                    ...
                    logger.warning(
                        "DSPARK fused KV append failed; falling back to sequential path: %s",
                        e,
                    )
                    self._use_fused_kv_materialize = False
                    self._fused_kv_helper = None
    ...
    def forward_batch_generation(
        ...
        **kwargs,
        ...
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            if (
                batch.extend_lens is None
                or batch.prefix_lens is None
            ):
                raise RuntimeError(
                    "DSPARK expected extend_lens / prefix_lens to be populated in extend mode, but got None."
                )
