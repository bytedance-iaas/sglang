...
DEFAULT_DSPARK_MASK_TOKEN = "<|MASK|>"

_DSPARK_SAMPLING_VERIFY_AVAILABLE = False
_DSPARK_CHAIN_VERIFY_BUFFERS: dict[tuple[Optional[int], int], dict[str, Any]] = {}
_DSPARK_VERIFY_SKIP_CUSTOM_MASK_BACKENDS = frozenset(
    {
...
if is_cuda() or is_musa():
    try:
        ...
        _DSPARK_SAMPLING_VERIFY_AVAILABLE = True
    except Exception:
...
def is_dspark_sampling_verify_available() -> bool:
    return _DSPARK_SAMPLING_VERIFY_AVAILABLE


def scale_kv_cell_size_per_token_for_dspark(
    *,
    ...
    """Compute bytes/token budget for combined target+draft KV pools (DSPARK).

    DSPARK runs a separate draft runner with its own KV pool. The target runner's
...
def resolve_dspark_verify_mask_policy(attn_backend: Any) -> tuple[str, bool]:
    ...
    return backend_name, (backend_name not in _DSPARK_VERIFY_SKIP_CUSTOM_MASK_BACKENDS)
...
def _get_or_create_chain_verify_buffers(
    *,
    ...
    cached = _DSPARK_CHAIN_VERIFY_BUFFERS.get(key)
    ...
    if cap_bs < bs:
        ...
        _DSPARK_CHAIN_VERIFY_BUFFERS[key] = cached
...
def build_target_layer_ids(num_target_layers: int, num_draft_layers: int) -> List[int]:
    """Select target layer indices used to build DSpark context features.

    Args:
        num_target_layers: Number of transformer layers in the runtime target model.
        num_draft_layers: Number of layers in the DSpark draft model.

    Returns:
        A list of 0-based target layer indices of length `num_draft_layers`.

    Notes:
        - DSpark uses hidden states after each selected target layer (HF-style).
    ...
    if end < start:
        raise ValueError(
            "DSpark layer selection requires num_target_layers >= 4. "
...
def _get_dspark_config(config: Any) -> dict:
    if isinstance(config, dict):
        cfg = config.get("dspark_config", None)
    else:
        cfg = getattr(config, "dspark_config", None)
    if cfg is None:
        return {}
...
class DSparkDraftConfig:
    ...
    def require_num_layers(self) -> int:
        if self.num_hidden_layers is None:
            raise ValueError(
                "DSPARK requires draft num_hidden_layers in config. "
    ...
    def resolve_target_layer_ids(
        ...
        *,
        ...
        if len(resolved) <= 0:
            raise ValueError(
                "DSPARK dspark_config.target_layer_ids must be non-empty. "
                f"Got len(target_layer_ids)={len(resolved)}."
            )
        for idx, val in enumerate(resolved):
            if val != -1 and (val < 0 or val >= target_num_layers):
                raise ValueError(
...
def parse_dspark_draft_config(*, draft_hf_config: Any) -> DSparkDraftConfig:
    """Parse and validate DSPARK draft config fields from HF config/dict."""
    dspark_cfg = _get_dspark_config(draft_hf_config)
    draft_text_config = _get_text_config(draft_hf_config)
    top_level_layer_ids = _cfg_get(
        draft_hf_config,
        "dspark_target_layer_ids",
        _cfg_get(draft_hf_config, "target_layer_ids", None),
    )

    raw_num_hidden_layers = dspark_cfg.get(
        "num_hidden_layers",
        _cfg_get(
            draft_hf_config,
            "dspark_num_hidden_layers",
            (
                len(top_level_layer_ids)
                if isinstance(top_level_layer_ids, (list, tuple))
                else _cfg_get(
                    draft_hf_config,
    ...
    if layer_ids is None:
        ...
        if not isinstance(layer_ids, (list, tuple)):
            raise ValueError(
                "DSPARK dspark_config.target_layer_ids must be a list of ints, "
    ...
    mask_token = dspark_cfg.get("mask_token", None)
    if mask_token is None:
        mask_token = DEFAULT_DSPARK_MASK_TOKEN
    if not isinstance(mask_token, str) or not mask_token:
        raise ValueError(
            "DSPARK dspark_config.mask_token must be a non-empty string, "
            f"got {mask_token!r}."
        )

    mask_token_id = dspark_cfg.get(
        "mask_token_id",
        _cfg_get(
            draft_hf_config,
    ...
    if mask_token_id is not None:
        if not isinstance(mask_token_id, Integral) or isinstance(mask_token_id, bool):
            raise ValueError(
                "DSPARK dspark_config.mask_token_id must be an integer, "
                f"got {mask_token_id!r} (type={type(mask_token_id).__name__})."
            )
        mask_token_id = int(mask_token_id)
        if mask_token_id < 0:
            raise ValueError(
                "DSPARK dspark_config.mask_token_id must be non-negative, "
                f"got {mask_token_id}."
            )
...
def can_dspark_slice_qkv_weight(qkv_proj: Any) -> Tuple[bool, str]:
    """Validate whether DSpark can slice KV weights from a fused QKV linear layer."""
...
def can_dspark_use_fused_qkv_proj(qkv_proj: Any) -> Tuple[bool, str]:
    """Validate whether a QKV layer is eligible for DSpark fused KV materialization."""
    eligible, reason = can_dspark_slice_qkv_weight(qkv_proj)
    if not eligible:
        return False, reason
...
def compute_dspark_correct_drafts_and_bonus(
    *,
    candidates: torch.Tensor,
    target_predict: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Compute DSpark accept lengths and bonus tokens (greedy verify rule).

    Args:
        candidates: Token ids proposed by the DSpark draft, including the current token.
...
def compute_dspark_sampling_correct_drafts_and_bonus(
    *,
    candidates: torch.Tensor,
    ...
    """Compute DSpark accept lengths and bonus tokens for non-greedy sampling.

    This is a chain-specialized variant of speculative target-only verification:
      - DSpark proposals are linear (topk == 1), so each verify level has at most one candidate.
    ...
    """
    if not _DSPARK_SAMPLING_VERIFY_AVAILABLE:
        raise RuntimeError(
            "DSPARK non-greedy verification is unavailable on this build/device."
        )
    if candidates.ndim != 2:
        raise ValueError(f"candidates must be 2D, got shape={tuple(candidates.shape)}")
