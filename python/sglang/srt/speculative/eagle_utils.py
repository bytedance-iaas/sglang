...
def eagle_prepare_for_decode(batch: ScheduleBatch):
    ...
    if (
        ...
        logical_lens_cpu = batch.seq_lens_cpu
        if logical_lens_cpu is None:
            logical_lens_cpu = batch.seq_lens.cpu()
        req_pool_indices_cpu = batch.req_pool_indices_cpu
        if req_pool_indices_cpu is None:
            req_pool_indices_cpu = batch.req_pool_indices.cpu()
        hisparse_coordinator.advance_dynamic_residency(
            [int(req_idx) for req_idx in req_pool_indices_cpu.tolist()],
            logical_lens_cpu,
        )
        ...
        if (
            ...
            # The target owns allocation from the shared logical allocator.
            # Mirror only the newly grown numerical slots into the draft
            # coordinator before it derives its physical write locations.
            for req_pool_idx in batch.req_pool_indices_cpu.tolist():
                draft_hisparse_coordinator.mirror_device_slots_from(
                    hisparse_coordinator, int(req_pool_idx)
                )
            draft_device_slots = (
                draft_hisparse_coordinator.get_draft_device_slots_variable(
                    batch.req_pool_indices,
                    nxt_kv_lens_cpu - cur_kv_lens_cpu,
                    cur_kv_lens_cpu,
                )
            )
            if not torch.equal(device_slots, draft_device_slots):
                raise RuntimeError(
                    "Target and draft HiSparse device-slot namespaces diverged"
                )
