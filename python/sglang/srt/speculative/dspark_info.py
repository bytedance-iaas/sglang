...
from sglang.srt.speculative.dspark_utils import (
    compute_dspark_correct_drafts_and_bonus,
...
class DSparkDraftInput(SpecInput):
    """Per-batch DSpark draft state for spec-v1 (non-overlap) scheduling.

    This object is stored on `ScheduleBatch.spec_info` between decode iterations.
    It is NOT sent to model attention backends; the DSpark worker uses it to run
    ...
    # Current token to start the next DSpark block (one per request).
    ...
    # hidden-state features concatenated per token (len(dspark_config.target_layer_ids),
...
class DSparkVerifyInput(SpecInput):
    """Inputs for a target-model verify forward in DSpark (spec-v1).
    ...
    draft_token_num: int
    # Maximum number of proposed DSpark tokens allowed by static confidence threshold.
    # Shape: [bs], values in [0, draft_token_num - 1]. None means no confidence cap.
    max_accept_lens: torch.Tensor | None = None
    # Kept for compatibility with attention backends that gate tree metadata by `topk > 1`.
    # DSPARK verify is linear (non-tree), so this is always 1.
    topk: int = 1
    ...
    def prepare_for_verify(
        ...
        *,
        ...
        if self.draft_token_num <= 0:
            raise ValueError(
                f"DSPARK draft_token_num must be positive, got {self.draft_token_num}."
            )
    ...
    def verify(
        ...
        *,
        ...
        """DSpark verification for greedy and non-greedy sampling.
        ...
        if sampling_info is not None:
            if len(sampling_info) != bs:
                raise RuntimeError(
                    "DSPARK verify sampling_info size mismatch: "
        ...
        if (
            ...
            and is_dspark_sampling_verify_available()
        ):
            correct_len, bonus = compute_dspark_sampling_correct_drafts_and_bonus(
            ...
            correct_len, bonus = compute_dspark_correct_drafts_and_bonus(
                candidates=candidates,
                target_predict=target_predict,
            )
        if self.max_accept_lens is not None:
            max_accept_lens = self.max_accept_lens.to(
                device=correct_len.device, dtype=correct_len.dtype
            )
            if max_accept_lens.shape != correct_len.shape:
                raise RuntimeError(
                    "DSPARK max_accept_lens shape mismatch: "
                    f"expected {tuple(correct_len.shape)}, got {tuple(max_accept_lens.shape)}."
                )
            capped_correct_len = torch.minimum(correct_len, max_accept_lens)
            if not torch.equal(capped_correct_len, correct_len):
                target_predict = torch.argmax(
                    logits_output.next_token_logits, dim=-1
                ).view(bs, self.draft_token_num)
                bonus = target_predict[
                    torch.arange(bs, device=target_predict.device), capped_correct_len
                ].to(torch.int64)
                correct_len = capped_correct_len
        ...
        if hidden is None:
            raise RuntimeError(
                "DSPARK verify requires target hidden states, but got None."
            )
