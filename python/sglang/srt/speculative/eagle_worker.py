...
class EAGLEWorker(TpModelWorker):
    ...
    def __init__(
        ...
        if server_args.speculative_draft_dp and server_args.enable_dp_attention:
            ...
            # Target's lm_head is full when --enable-dp-lm-head is set; otherwise
            # it is sharded by TP and must be allgathered.
        ...
        if server_args.speculative_draft_dp and server_args.enable_dp_attention:
            # Draft-DP + DP attention: keep TP as the global TP group during
            # forward so dp_gather_partial can gather DP-attention tokens.
            # MoE remains local via draft_dp_group.
            @contextmanager
