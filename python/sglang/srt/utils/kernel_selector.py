...
class KernelSelector:
    ...
    def __init__(self):
        ...
        try:
                ...
                for loader, module_name, is_pkg in pkgutil.iter_modules(
                    ...
                    if not is_pkg:
                        ...
                        if hasattr(submod, "kernel_data"):
                            self.all_kernel_data[module_name] = getattr(
                                submod, "kernel_data"
                            )
            ...
            warnings.warn(
                f"Failed to import 'kernel_select_data': {e}. Kernel selection will use defaults."
            )
            self.import_success = False
    ...
    def summary_status(self):
        ...
        for op_type in self.launch_times:
            ...
            for kernel in self.launch_data[op_type]:
                print(
                    "\t\t\t {} : {} ".format(kernel, self.launch_data[op_type][kernel])
                )
    ...
    def query_kernel_data(
        ...
        **kwargs,
        ...
        if get_bool_env_var("SHOW_SELECTOR_STATUS"):
            if (
                self.launch_times[op_type] % 1000 == 0
                and self.launch_times[op_type] > 0
            ):
                self.summary_status()
    ...
    def refresh_kernel_data(self, level1_key: str):
        ...
        if not self.import_success or not self.package_dir:
            print(
                "Cannot update: kernel_select_data module was not loaded successfully."
            )
            return
            ...
            try:
                ...
                if content.strip():
                    try:
                        ...
                        print(
                            f"Warning: Failed to parse existing file {file_path}: {e}"
                        )
