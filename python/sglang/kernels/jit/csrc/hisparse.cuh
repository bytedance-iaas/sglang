...
__global__ void load_cache_to_device_buffer_kernel(
  ...
  for (int64_t step = 0; step < num_steps; step++) {
    ...
    // Dynamic HiSparse resident rows keep full KV on device. In a mixed
    // batch, translate every step's selected logical positions and skip the
    // swap/LRU path while retaining the graph-stable req-major layout.
    if (full_to_hisparse_device_index_mapping != nullptr &&
        req_device_buffer_locs[0] < 0) {
      const int count = (seq_len < NUM_TOP_K) ? static_cast<int>(seq_len) : NUM_TOP_K;
      const int32_t* req_logical_locs = req_to_token + rid * req_to_token_stride_0;
      for (int i = tid; i < count; i += BLOCK_SIZE) {
        const int32_t token_pos = step_top_k_tokens[i];
        if (token_pos >= 0) {
          int32_t logical_loc;
          if constexpr (IsDsv4Layout) {
          ...
          step_top_k_device_locs[i] = static_cast<int32_t>(
              full_to_hisparse_device_index_mapping[logical_loc]);
        }
      }
      continue;
    }
    ...
    if (seq_len <= HOT_BUFFER_SIZE) {
      ...
      for (int i = tid; i < count; i += BLOCK_SIZE) {
        ...
        if (try_get_extra_page_device_loc<HOT_BUFFER_SIZE, IsDsv4Layout>(
                ...
                page_size,
                &loc) ||
            try_get_static_device_loc<HOT_BUFFER_SIZE, IsDsv4Layout>(
    ...
    for (int iter = 0; iter < ITERATIONS_PER_WARP_BUFFER; iter++) {
      ...
      if (warp_id == 0) {
        ...
        total_hit_count = warp_inclusive_scan(
            s_chunk_offset, lane_id, scan_offset, scan_count, total_hit_count);
        total_evict_count = warp_inclusive_scan(
            s_evict_chunk_offset, lane_id, scan_offset, scan_count, total_evict_count);
#else
        total_hit_count =
    ...
    for (int iter = 0; iter < ITERATIONS_PER_WARP_TOKEN; iter++) {
      ...
      if (warp_id == 0) {
        ...
        total_misses = warp_inclusive_scan(
            s_chunk_offset, lane_id, scan_offset, scan_count, total_misses);
#else
    ...
    for (int miss_idx = warp_id; miss_idx < total_misses; miss_idx += NUM_WARPS) {
      ...
      if constexpr (IsDsv4Layout) {
        ...
        const auto [dst_value_ptr, dst_scale_ptr] =
            get_pointer_paged(device_buffer_k, static_cast<int32_t>(dst_loc));
