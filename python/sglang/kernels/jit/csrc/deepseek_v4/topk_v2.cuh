...
struct TopKPagedParams {
  ...
  int32_t* __restrict__ raw_indices;      // optional selection before page transform
...
struct TopKKernel {
  ...
  static void transform_paged(
    ...
    if (raw_indices.has_value()) {
      ...
      TensorMatcher({B, K})  // optional raw selection before page transform
          .with_dtype<int32_t>()
          .with_device(device_)
    ...
    RuntimeCheck(
        raw_indices_ptr == nullptr || raw_indices_ptr != page_indices.data_ptr(),
        "raw_indices must not alias page_indices");
