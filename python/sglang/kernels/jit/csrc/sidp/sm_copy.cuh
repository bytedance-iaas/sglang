...
constexpr int32_t kSmTraceFields = 6;

// Opt-in execution trace used by the headless pipeline profiler.  Production
// entry points continue to launch the original kernels, so the normal hot path
// has neither a trace branch nor additional kernel arguments.  One row is:
//   tag, blockIdx.x, entry_smid, exit_smid, entry_globaltimer, exit_globaltimer
// `%smid` is volatile under preemption, hence both entry and exit are kept.
struct SmExecutionTrace {
  int64_t* rows;
  int32_t* count;
  int32_t* overflow;
  int32_t capacity;
...
__device__ __forceinline__ int32_t read_smid() {
  uint32_t value;
  asm volatile("mov.u32 %0, %%smid;" : "=r"(value));
  return static_cast<int32_t>(value);
}

__device__ __forceinline__ int32_t begin_sm_trace(const SmExecutionTrace& trace) {
  if (threadIdx.x != 0) return -1;
  const int32_t index = atomicAdd(trace.count, 1);
  if (index >= trace.capacity) {
    atomicExch(trace.overflow, 1);
    return -1;
  }
  int64_t* row = trace.rows + static_cast<int64_t>(index) * kSmTraceFields;
  row[0] = trace.tag;
  row[1] = static_cast<int64_t>(blockIdx.x);
  row[2] = read_smid();
  row[3] = -1;
  row[4] = static_cast<int64_t>(read_globaltimer());
  row[5] = 0;
  return index;
}

__device__ __forceinline__ void end_sm_trace(
    const SmExecutionTrace& trace, int32_t index) {
  if (threadIdx.x != 0 || index < 0 || index >= trace.capacity) return;
  int64_t* row = trace.rows + static_cast<int64_t>(index) * kSmTraceFields;
  row[3] = read_smid();
  row[5] = static_cast<int64_t>(read_globaltimer());
}

__global__ void reset_sm_trace_kernel(int32_t* count, int32_t* overflow) {
...
__global__ void reset_forward_state_traced_kernel(
    int32_t* fill_gen,
    int32_t* comp_gen,
    int32_t num_slots,
    int32_t resident_slots,
...
__global__ void reset_cycle_state_traced_kernel(
    uint8_t* done,
    int32_t count,
    int32_t* selected,
    int64_t* claim_spins,
...
__global__ void select_fixed_traced_kernel(
    int32_t* selected, int32_t index, SmExecutionTrace trace) {
  const int32_t trace_index = begin_sm_trace(trace);
  if (blockIdx.x == 0 && threadIdx.x == 0) selected[0] = index;
  end_sm_trace(trace, trace_index);
}

constexpr int32_t kClaimOrderRotating = 0;
constexpr int32_t kClaimOrderComputePriority = 1;
...
__global__ void claim_owner_kernel(
  ...
  int32_t start = 0;
  if (claim_order == kClaimOrderRotating) {
    start = probe_cursor[0];
    if (start < 0) start = 0;
    start %= candidate_count;
  } else if (claim_order != kClaimOrderComputePriority) {
...
__global__ void claim_owner_traced_kernel(
    const uint64_t* owner_state_ptrs,
    int32_t owner_count,
    const int32_t* candidate_owners,
    const int32_t* candidate_slots,
...
__global__ void copy_selected_traced_kernel(
    const uint64_t* src_ptrs,
    const uint64_t* dst_ptrs,
    const int64_t* sizes,
    int32_t candidate_count,
...
__global__ void release_owner_traced_kernel(
    const uint64_t* owner_state_ptrs,
    int32_t owner_count,
    const int32_t* candidate_owners,
    int32_t candidate_count,
...
__global__ void publish_selected_fill_traced_kernel(
    int32_t* fill_gen,
    const int32_t* candidate_slots,
    int32_t candidate_count,
    const int32_t* selected,
...
__global__ void wait_generation_traced_kernel(
    int32_t* generations,
    int32_t slot,
    int32_t target,
    int32_t backoff_ns,
...
__global__ void publish_generation_traced_kernel(
    int32_t* generations,
    int32_t slot,
    int32_t target,
    SmExecutionTrace trace) {
...
struct SidpSmCopyKernels {
  static SmExecutionTrace validate_sm_trace(
      tvm::ffi::TensorView rows,
      tvm::ffi::TensorView count,
      tvm::ffi::TensorView overflow,
      int64_t tag,
      host::SymbolicDevice& device) {
    using namespace host;
    auto R = SymbolicSize{"trace_capacity"};
    TensorMatcher({R, kSmTraceFields})
        .with_dtype<int64_t>()
    ...
    TensorMatcher({1})
        ...
        .verify(count)
        .verify(overflow);
    RuntimeCheck(R.unwrap() > 0, "SiDP SM trace capacity must be positive");
    RuntimeCheck(
        tag >= 0 && tag <= std::numeric_limits<int32_t>::max(),
        "invalid SiDP SM trace tag");
    return {
        static_cast<int64_t*>(rows.data_ptr()),
        static_cast<int32_t*>(count.data_ptr()),
        static_cast<int32_t*>(overflow.data_ptr()),
        static_cast<int32_t>(R.unwrap()),
        static_cast<int32_t>(tag)};
  }

  static void reset_sm_trace(
      tvm::ffi::TensorView count, tvm::ffi::TensorView overflow) {
    using namespace host;
  ...
  static void reset_forward_state_traced(
      tvm::ffi::TensorView fill_gen,
      tvm::ffi::TensorView comp_gen,
      int64_t resident_slots,
      tvm::ffi::TensorView error_state,
      tvm::ffi::TensorView trace_rows,
      tvm::ffi::TensorView trace_count,
      tvm::ffi::TensorView trace_overflow,
      int64_t trace_tag) {
    using namespace host;
    auto N = SymbolicSize{"num_slots"};
    auto device = SymbolicDevice{};
    ...
    TensorMatcher({1}).with_dtype<int32_t>().with_device<kDLGPU>(device).verify(error_state);
    RuntimeCheck(resident_slots >= 0 && resident_slots <= N.unwrap(), "invalid resident slot count");
    const auto trace = validate_sm_trace(
        trace_rows, trace_count, trace_overflow, trace_tag, device);
    LaunchKernel(div_ceil(N.unwrap(), int64_t{256}), 256, device.unwrap())(
  ...
  static void reset_cycle_state_traced(
      tvm::ffi::TensorView done,
      tvm::ffi::TensorView selected,
      tvm::ffi::TensorView claim_spins,
      tvm::ffi::TensorView claim_collisions,
  ...
  static void select_fixed_traced(
      tvm::ffi::TensorView selected,
      int64_t index,
      tvm::ffi::TensorView trace_rows,
      tvm::ffi::TensorView trace_count,
  ...
  static void claim_owner(
    ...
    LaunchKernel(1, 1, device.unwrap())(
        ...
        static_cast<int32_t>(claim_order),
  ...
  static void claim_owner_traced(
      tvm::ffi::TensorView owner_state_ptrs,
      tvm::ffi::TensorView candidate_owners,
      tvm::ffi::TensorView candidate_slots,
      tvm::ffi::TensorView done,
  ...
  static void copy_selected(
    ...
    RuntimeCheck(
        C.unwrap() > 0 && grid_blocks > 0 &&
            (block_size == 128 || block_size == 256 || block_size == 512),
        "invalid SiDP SM copy launch dimensions");
    LaunchKernel(
  ...
  static void copy_selected_traced(
      tvm::ffi::TensorView src_ptrs,
      tvm::ffi::TensorView dst_ptrs,
      tvm::ffi::TensorView sizes,
      tvm::ffi::TensorView selected,
  ...
  static void release_owner_traced(
      tvm::ffi::TensorView owner_state_ptrs,
      tvm::ffi::TensorView candidate_owners,
      tvm::ffi::TensorView selected,
      int64_t requester_rank,
  ...
  static void publish_selected_fill_traced(
      tvm::ffi::TensorView fill_gen,
      tvm::ffi::TensorView candidate_slots,
      tvm::ffi::TensorView selected,
      int64_t target_gen,
  ...
  static void wait_generation_traced(
      tvm::ffi::TensorView generations,
      int64_t slot,
      int64_t target,
      int64_t backoff_ns,
  ...
  static void publish_generation_traced(
      tvm::ffi::TensorView generations,
      int64_t slot,
      int64_t target,
      tvm::ffi::TensorView trace_rows,
