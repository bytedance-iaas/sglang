...
def topk_transform_paged_v2(
    ...
    * Both outputs given -- ``out_page_indices`` receives the page-table
      transform and ``out_raw_indices`` receives the selected sequence-relative
      indices from the same kernel invocation.

    ``out_raw_indices`` is valid only with ``page_tables`` and must not alias
    ``out_page_indices``. It exists for graph-safe diagnostics; normal serving
    leaves it unset.
