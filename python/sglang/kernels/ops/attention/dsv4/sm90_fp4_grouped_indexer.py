...
def _prefer_triton(rows: int, width: int) -> bool:
    # H20 crossovers for partial/full six-query speculative groups. A single
    # row has no cross-query K reuse; 5/6 rows also require a second MMA round.
    if rows <= 6:
        minimum_cuda_width = (0, 32768, 3072, 3584, 3072, 4096, 3584)
        return width < minimum_cuda_width[rows]
    return rows * width <= 20480
...
def fp4_index_logits_grouped_sm90(
    ...
    if not q.is_cuda or any(t.device != q.device for t in tensors):
        raise ValueError("All indexer tensors must be on the same CUDA device")
    ...
    if req_to_token.shape[1] < width * ratio or table.shape[1] < page_size * 68:
        raise ValueError("Mapping/cache shape is too small for width/page_size")
    if q.shape[0] == 0 or width == 0:
        return torch.empty((q.shape[0], width), dtype=torch.float32, device=q.device)
    ...
    if q.shape[1] == 32 or _prefer_triton(q.shape[0], width):
        ...
        return fp4_index_logits_mapped_sm90(
            q,
            weights,
            req_to_token,
            req,
    ...
    from sglang.kernels.ops.attention.dsv4.sm90_fp4_query_pack import pack_queries

    q_fp4, q_scale = pack_queries(q)
