"""SM90 native-H16 BF16 sparse MLA for 512-wide Q/K/V.

Adapted from SGLang PR #36059 (a9996143da29d6ed9ccceed4bab4f742086436f9).
The transposed m64n16 QK/PV decomposition avoids padding 16 logical heads
into a 64-head MMA. Each query has independent pre-masked sparse indices.
Invalid indices are zero-filled before either matrix multiply; per-query
lengths and an optional virtual zero-value sink preserve sparse semantics.

This entry point requires exactly 16 logical heads. Cache conversion and
model/backend dispatch are owned by the caller. It returns [TQ, 16, 512].
"""
...
# Native logical head count and shared Q/K/V width.
H = 16
DQK = 512
DV = 512
BN = 64
...
# (nine chunks); 512-wide MLA only needs 64 and saves 9 KiB of shared memory.
...
def _kernel(
    ...
    # Each query owns exactly H logical heads.
...
def _validate_inputs(
    ...
    """Validate the native-H16 launch contract without tensor allocations."""

    if q.ndim != 3 or q.shape[1] != H or q.shape[2] != DQK:
        raise ValueError(f"q must have shape [TQ, 16, 512], got {tuple(q.shape)}")
    ...
    if capability != (9, 0):
        raise ValueError(
            "sparse_mla_h16_fwd requires a Hopper SM90 GPU, "
    ...
    # Keep a contiguous view of the logical heads when the sink is padded.
...
def sparse_mla_h16_fwd(
    ...
    """Run native-H16 sparse prefill and return ``[TQ, 16, 512]``."""
