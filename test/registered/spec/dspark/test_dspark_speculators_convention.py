...
class TestDsparkSpeculatorsConventionDetection(CustomTestCase):
    ...
    def test_explicit_false_overrides_speculators_model_type(self):
        # The internal GLM-5.2 checkpoint has exactly this combination:
        # speculators metadata, but the legacy anchor-first layout.
        config = parse_dspark_draft_config(
            draft_hf_config=_base_dspark_hf_config(
                speculators_model_type="dspark", dspark_bonus_anchor=False
            )
        )
        self.assertFalse(config.bonus_anchor)

    def test_explicit_true_enables_bonus_anchor(self):
        config = parse_dspark_draft_config(
            draft_hf_config=_base_dspark_hf_config(dspark_bonus_anchor=True)
        )
        self.assertTrue(config.bonus_anchor)

    def test_sample_from_anchor_is_inverse_fallback(self):
        for sample_from_anchor, expected_bonus_anchor in ((True, False), (False, True)):
            with self.subTest(sample_from_anchor=sample_from_anchor):
                config = parse_dspark_draft_config(
                    draft_hf_config=_base_dspark_hf_config(
                        sample_from_anchor=sample_from_anchor
                    )
                )
                self.assertEqual(config.bonus_anchor, expected_bonus_anchor)

    def test_explicit_bonus_anchor_precedes_sample_from_anchor(self):
        config = parse_dspark_draft_config(
            draft_hf_config=_base_dspark_hf_config(
                dspark_bonus_anchor=False, sample_from_anchor=False
            )
        )
        self.assertFalse(config.bonus_anchor)

    def test_non_boolean_explicit_convention_rejected(self):
        for field, value in (
            ("dspark_bonus_anchor", "false"),
            ("sample_from_anchor", 1),
        ):
            with self.subTest(field=field):
                with self.assertRaisesRegex(ValueError, "must be a boolean"):
                    parse_dspark_draft_config(
                        draft_hf_config=_base_dspark_hf_config(**{field: value})
                    )
