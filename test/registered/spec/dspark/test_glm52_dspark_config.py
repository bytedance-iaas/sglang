...
def _glm52_dspark_config_dict() -> dict:
    return {
...
def _glm52_dspark_config() -> SimpleNamespace:
    return _to_namespace(_glm52_dspark_config_dict())


class TestGLM52DSparkConfig(CustomTestCase):
    ...
    def test_dspark_parser_prefers_speculators_tokens_for_gamma(self):
        ...
        self.assertFalse(parsed.bonus_anchor)

    def test_dspark_parser_selects_default_proposal_method(self):
        raw_config = _glm52_dspark_config_dict()
    ...
    def test_hf_loader_falls_back_for_missing_top_level_model_type(self):
        ...
        parsed = parse_dspark_draft_config(draft_hf_config=loaded)
        self.assertEqual(parsed.gamma, 7)
        self.assertFalse(parsed.bonus_anchor)
