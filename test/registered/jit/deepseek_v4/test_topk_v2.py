...
from sglang.jit_kernel.deepseek_v4 import plan_topk_v2, topk_transform_512_v2
...
@pytest.mark.parametrize("k", [64, 512, 1024])
@pytest.mark.parametrize("batch,seq", [(4, 4096), (8, 131072)])
@torch.inference_mode()
def test_topk_v2_raw_indices_only(batch: int, seq: int, k: int) -> None:
    """HiSparse C4 swap-in consumes raw token positions and intentionally skips
    the page-table output. The page-index buffer should stay invalid while raw
    indices still match the selected top-k positions."""
    torch.manual_seed(batch * 1009 + seq * 13 + k)
    device = "cuda"
    width = (seq + 3) & ~3
    scores = torch.randn(batch, width, dtype=torch.float32, device=device)[:, :seq]
    seq_lens = torch.full((batch,), seq, dtype=torch.int32, device=device)
    ...
    topk_transform_512_v2(
        ...
        metadata,
        raw,
        raw_indices_only=True,
    )
    torch.cuda.synchronize()

    assert torch.all(out == -1)
    raw_cpu = raw.cpu().tolist()
    our_raw = [[v for v in raw_cpu[i] if v != -1] for i in range(batch)]
    ref_raw = _reference(scores, seq_lens, k)
    _assert_topk_close(scores.cpu(), ref_raw, our_raw, batch, seq_lens.cpu(), k)
