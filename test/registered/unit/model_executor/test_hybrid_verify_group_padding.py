...
def _prepare(
    ...
    *,
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch("sglang.srt.layers.cp.utils.enable_cp_v2", return_value=False)
        )
        stack.enter_context(
            patch(
                "sglang.srt.layers.cp.padding.get_cp_padding_align_size",
                return_value=cp_alignment,
            )
        )
...
class TestHybridVerifyGroupPadding(CustomTestCase):
    ...
    def test_legacy_cp_alignment_preserves_whole_request_groups(self):
        fb = _batch(batch_size=33, spec=_spec())
        _prepare(fb, _model_runner(), cp_alignment=16)
        self.assertEqual(fb.global_num_tokens_cpu, [240])
        self.assertEqual(fb.batch_size, 40)
        self.assertEqual(fb.input_ids.numel() % 16, 0)
        self.assertEqual(fb.input_ids.numel() % 6, 0)
