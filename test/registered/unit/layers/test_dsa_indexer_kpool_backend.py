...
class TestKPoolMqaBackend(CustomTestCase):
    def _run_paged_mqa_dispatch(self, forward_mode, metadata_rows=None):
        ...
        pool_block_tables = torch.arange(n_real * 2, dtype=torch.int32).view(n_real, 2)
        ...
        metadata_block_tables = torch.arange(metadata_rows * 2, dtype=torch.int32).view(
            metadata_rows, 2
        )
        metadata = SimpleNamespace(
            attn_metadata=SimpleNamespace(kpool_write_plan=plan),
            get_page_table_64=lambda: metadata_block_tables,
            get_seqlens_expanded=lambda: torch.arange(metadata_rows, dtype=torch.int32),
            get_seqlens_int32=lambda: torch.arange(batch_size, dtype=torch.int32),
        )
        forward_batch = SimpleNamespace(
            forward_mode=forward_mode, global_num_token_non_padded_cpu=n_real
        )
        q_fp8 = torch.empty(num_q_padded, num_heads, 128, dtype=torch.float8_e4m3fn)
        ...
        deep_gemm.fp8_paged_mqa_logits.return_value = torch.empty(n_real, max_seq_len)
    ...
    def test_draft_extend_cache_write_uses_pre_padding_plan(self):
        ...
        forward_batch.global_num_token_non_padded_cpu = 6
