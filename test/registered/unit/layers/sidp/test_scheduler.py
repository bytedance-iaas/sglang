...
@pytest.mark.parametrize("slice_count", [1, 2, 3, 4, 7])
def test_dma_slice_ranges_are_exact(slice_count):
    ranges = dma_slice_ranges(101, slice_count)
    assert ranges[0][0] == 0
    assert ranges[-1][1] == 101
    assert all(left[1] == right[0] for left, right in zip(ranges, ranges[1:]))
    assert sum(end - start for start, end in ranges) == 101
    assert max(end - start for start, end in ranges) - min(
        end - start for start, end in ranges
    ) <= 1


def test_dma_slice_groups_match_benchmark_semantics():
    layers = list(range(7))
    assert contiguous_slice_groups(layers, 1) == [tuple(range(7))]
    assert contiguous_slice_groups(layers, 2) == [(0, 1, 2, 3), (4, 5, 6)]
    assert contiguous_slice_groups(layers, 4) == [
        (0, 1),
        (2, 3),
        (4, 5),
        (6,),
    ]
    assert contiguous_slice_groups(layers, 20) == [(layer,) for layer in layers]


def test_dma_slice_ranges_allow_more_slices_than_bytes():
    ranges = dma_slice_ranges(2, 4)
    assert ranges == [(0, 0), (0, 1), (1, 1), (1, 2)]
    assert sum(end - start for start, end in ranges) == 2


def test_dma_slice_plan_rejects_invalid_counts():
    with pytest.raises(ValueError, match="positive"):
        dma_slice_ranges(100, 0)
    with pytest.raises(ValueError, match="non-negative"):
        dma_slice_ranges(-1, 2)
    with pytest.raises(ValueError, match="positive"):
        contiguous_slice_groups([1, 2], 0)
