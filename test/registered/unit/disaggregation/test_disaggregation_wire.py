...
class TestEagleDsaSeedTransfer(unittest.TestCase):
    ...
    def test_decode_input_can_ignore_transferred_seed_for_diagnosis(self):
        seed = torch.tensor([1, 2, 3], dtype=torch.int32)
        batch = SimpleNamespace(
            reqs=[self._make_req(seed)],
            device="cpu",
            enable_overlap=False,
        )
        server_args = SimpleNamespace(
        ...
        with envs.SGLANG_TEST_IGNORE_PD_DSA_TOPK_SEED.override(True):
            draft_input = build_eagle_disagg_draft_input(
                batch, server_args, torch.tensor([11]), None
            )
