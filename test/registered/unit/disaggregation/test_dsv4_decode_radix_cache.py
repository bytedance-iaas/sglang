...
def test_dsv4_prompt_insert_uses_prompt_snapshot_and_restores_fill_ids():
    req = SimpleNamespace(
        ...
        kv=SimpleNamespace(swa_evicted_seqlen=4),
    )
    req.get_fill_ids = lambda: req.full_untruncated_fill_ids
    ...
    def cache_unfinished_req(inserted_req, **_kwargs):
        ...
        tree_cache.inserted_swa_evicted_seqlens.append(
            inserted_req.kv.swa_evicted_seqlen
        )
    ...
    assert req.kv.swa_evicted_seqlen == 4
    assert req.allow_radix_cache_insert_once is False
...
def test_dsv4_decode_radix_match_uses_full_match_for_full_only_leaf():
    ...
    prefix_match = DecodePreallocQueue._match_prefix_and_lock(queue, req)

    assert prefix_match.prefix_indices.tolist() == [1, 2]
    assert prefix_match.l1_prefix_len == 2
