...
def test_pp_disagg_output_ring_relays_explicit_skip_marker():
    events = []
    skip_tensors = {"__skip__": True}
    send_work = [object()]
    target = SimpleNamespace(forward_mode=SimpleNamespace(is_prebuilt=lambda: False))
    scheduler = SimpleNamespace(
        pp_group=SimpleNamespace(is_last_rank=False),
        ...
        device_module=SimpleNamespace(
            Event=Mock(return_value=object()),
            current_stream=Mock(return_value=object()),
        ),
        _pp_recv_dict_from_prev_stage=Mock(return_value=skip_tensors),
        _pp_make_skip_output_result=Mock(return_value=(None, object(), object())),
        _pp_send_dict_to_next_stage=Mock(
            side_effect=lambda tensors, async_send, msg_type: events.append(
                ("send", tensors, async_send, msg_type)
            )
    ...
    assert outputs.tensors is skip_tensors
    assert work == []
    assert events == [
        ("send", skip_tensors, True, "output"),
        ("commit", send_work),
    ]
    scheduler.schedule_stream.synchronize.assert_not_called()


def test_pp_disagg_output_ring_uses_sender_skip_decision():
    target = SimpleNamespace(forward_mode=SimpleNamespace(is_prebuilt=lambda: False))
    skip_tensors = {"__skip__": True}
    batch_result = object()
    d2h_event = Mock()
    scheduler = SimpleNamespace(
        ...
        _pp_recv_dict_from_prev_stage=Mock(return_value=skip_tensors),
        _pp_make_skip_output_result=Mock(return_value=(None, batch_result, d2h_event)),
    ...
    assert event is d2h_event
    assert work == []


def test_pp_disagg_output_ring_receives_real_output_despite_local_skip():
    target = SimpleNamespace(forward_mode=SimpleNamespace(is_prebuilt=lambda: False))
    received_tensors = {"next_token_ids": object()}
    batch_result = object()
    d2h_event = Mock()
    scheduler = SimpleNamespace(
        ...
        _pp_make_skip_output_result=Mock(),
    ...
    assert event is d2h_event
    assert work == []
    scheduler._pp_make_skip_output_result.assert_not_called()
    scheduler._pp_prep_batch_result.assert_called_once()


def test_pp_disagg_output_ring_last_stage_sends_explicit_skip_marker():
    target = SimpleNamespace(forward_mode=SimpleNamespace(is_prebuilt=lambda: False))
    queued_outputs = SimpleNamespace(tensors={"next_token_ids": object()})
    q_event = object()
    send_work = [object()]
    scheduler = SimpleNamespace(
        pp_group=SimpleNamespace(is_last_rank=True),
        device_module=SimpleNamespace(
            current_stream=Mock(return_value=SimpleNamespace(wait_event=Mock()))
        ),
        _pp_send_dict_to_next_stage=Mock(return_value=send_work),
    )
    ...
    scheduler.device_module.current_stream.return_value.wait_event.assert_not_called()
