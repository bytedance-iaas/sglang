...
class TestEaglePPLastRankDraftOwnership(unittest.TestCase):
    ...
    def test_last_prefill_records_target_sample_before_post_draft_extend(self):
        sampled = torch.tensor([8451], dtype=torch.int64)
        next_draft_input = SimpleNamespace(bonus_tokens=sampled)
        output = SimpleNamespace(
            next_token_ids=sampled,
            logits_output=SimpleNamespace(
                hidden_states=torch.ones((1, 2)), mm_input_embeds=None
            ),
            next_draft_input=None,
            new_seq_lens=None,
        )
        probe = SimpleNamespace(
            record_prefill_target_sample=MagicMock(),
            record_prefill_post_draft_extend=MagicMock(),
        )
        draft_worker = SimpleNamespace(
            eagle_numerical_probe=probe,
            draft_runner=SimpleNamespace(tp_group=object()),
            draft_tp_context=MagicMock(return_value=nullcontext()),
            _draft_extend_for_prefill=MagicMock(return_value=next_draft_input),
        )
        worker = SimpleNamespace(
            _pp_enabled=True,
            _pp_is_last_rank=True,
            _draft_worker=draft_worker,
            draft_worker=draft_worker,
            speculative_algorithm=SimpleNamespace(is_standalone=lambda: False),
            target_worker=SimpleNamespace(
                forward_batch_generation=MagicMock(return_value=output)
            ),
        )
        ...
        events = MagicMock()
        events.attach_mock(probe.record_prefill_target_sample, "target_sample")
        events.attach_mock(draft_worker._draft_extend_for_prefill, "draft_extend")
        events.attach_mock(probe.record_prefill_post_draft_extend, "post_draft_extend")
        ...
        self.assertEqual(
            [str(call).split("(", maxsplit=1)[0] for call in events.mock_calls],
            ["call.target_sample", "call.draft_extend", "call.post_draft_extend"],
        )
        probe.record_prefill_target_sample.assert_called_once_with(
            rids=["probe-rid"], next_token_ids=sampled
        )
        probe.record_prefill_post_draft_extend.assert_called_once_with(
            rids=["probe-rid"],
            next_token_ids=sampled,
