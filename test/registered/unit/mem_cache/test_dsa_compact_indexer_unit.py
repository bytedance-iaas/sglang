...
class TestDsaCompactIndexerMask(CustomTestCase):
    ...
    def test_invalid_partial_pool_returns_none(self):
        with patch.dict(os.environ, {_ENV: "1"}):
            cfg = _cfg(num_hidden_layers=8, index_topk_freq=4)
    ...
    def test_pp_stage_uses_model_global_pattern(self):
        with patch.dict(os.environ, {_ENV: "1"}):
            cfg = _cfg(
                num_hidden_layers=8,
            ...
            self.assertEqual(
                dsa_compact_indexer_layer_mask(cfg, 4, 0, 4),
                [True, False, False, True],
            )
            self.assertEqual(
                dsa_compact_indexer_layer_mask(cfg, 4, 4, 8),
                [True, False, True, False],
            )

    def test_draft_worker_never_compacts(self):
        with patch.dict(os.environ, {_ENV: "1"}):
            cfg = _cfg(num_hidden_layers=8, index_topk_freq=4)
            self.assertIsNone(
                dsa_compact_indexer_layer_mask(
                    cfg, 4, 0, 4, is_draft_worker=True
                )
            )
