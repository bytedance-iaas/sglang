...
class TestHybridIndexSidecar(unittest.TestCase):
    ...
    def test_strategy_registers_stage_local_index_transfer_spec(self):
        for start_layer in (0, 24):
            with self.subTest(start_layer=start_layer):
                pool = dsa_pool()
                entry, _ = self.build_entry(pool)
                group = MagicMock()
                group.entry_map = {PoolName.INDEXER: entry}
                params = MagicMock()
                params.req_to_token_pool.mamba_map = {
                    start_layer + 1: 0,
                    start_layer + 2: 1,
                }
                ...
                with patch(
                    self.prefix + "build_hybrid_mamba_stack",
                ...
                self.assertEqual(
                    build.call_args.kwargs["full_layer_mapping"], {0: 0, 3: 1}
                )
                self.assertEqual(
                    build.call_args.kwargs["mamba_layer_mapping"], {1: 0, 2: 1}
                )
