...
def _pool(dtype, dim, live, compressed, rope_dim=64):
...
def test_dsa_transfer_bytes(layout, backend, mode):
    ...
    """
    hybrid = mode.startswith("hybrid_")
    if hybrid and (layout, backend) != ("page_first_direct", "direct"):
        pytest.skip("Hybrid KDA regression uses the supported deployment layout")
    publish(
        ServerArgs(
            model_path="dummy",
    ...
    try:
        ...
        start_layer = 24 if mode == "hybrid_last" else 0
        stage_layers = 21 if start_layer else 24
        full_ids = list(range(start_layer, start_layer + stage_layers, 3))
        mamba_ids = [
            i
            for i in range(start_layer, start_layer + stage_layers)
            if i not in full_ids
        ]
        live = (
            tuple(i % 3 != 1 for i in range(len(full_ids)))
            if hybrid
        ...
        target = _pool(
            torch.float8_e4m3fn,
            512 if hybrid else 656,
            live,
            compressed,
            rope_dim=0 if hybrid else 64,
        )
        if mode in ("no_draft", "hybrid_first"):
            drafts = ()
        elif mode in ("packed", "hybrid_last"):
            drafts = tuple(
                _pool(
                    torch.float8_e4m3fn,
                    512 if hybrid else 656,
                ...
                for _ in range(1 if hybrid else 2)
            )
        else:
            drafts = (
        ...
        target_runner = SimpleNamespace(
            token_to_kv_pool=kvcache, spec_algorithm=SpeculativeAlgorithm.EAGLE
        )
        runners = tuple(
            SimpleNamespace(
                token_to_kv_pool=p,
        ...
        if hybrid:
            params.req_to_token_pool = SimpleNamespace(
                mamba_pool=mamba,
                mamba_map={layer: index for index, layer in enumerate(mamba_ids)},
                mamba_allocator=SimpleNamespace(
                    alloc=lambda size: (
                        torch.tensor([10, 12], device="cuda") if size == 2 else None
                    ),
        ...
        with (
            ...
            assembled = (_MambaStrategy() if hybrid else _DsaStrategy()).build(
                cache=None,
                kvcache=kvcache,
                params=params,
                server_args=None,
            ...
            controller.layer_num = stage_layers if hybrid else target.layer_num
            ...
            if drafts and plan.mode == HiCacheDraftMode.SIDECAR:
        ...
        mamba_checks = []
        if hybrid:
            mamba_host = group.get_pool(PoolName.MAMBA)
            for device, host in zip(
                (mamba.mamba_cache.temporal, *mamba.mamba_cache.conv),
                (mamba_host.temporal_buffer, *mamba_host.conv_buffer),
                strict=True,
            ):
                original = torch.randint(
                    256,
                    device.view(torch.uint8).shape,
                    dtype=torch.uint8,
                    generator=generator,
                )
                device.view(torch.uint8).copy_(original)
                host.view(torch.uint8).fill_(91)
                originals[id(device)] = (device, original)
                mamba_checks.append(
                    (device, host, torch.full_like(host.view(torch.uint8), 91))
                )
        ...
        for start in (0, 4):
            ...
            if hybrid:
                extras.append(
                    PoolTransfer(
                        PoolName.MAMBA,
                        device_indices=torch.tensor([2 + start], device="cuda"),
                    )
                )
            ...
            if hybrid:
                mamba_transfer = next(
                    t for t in op.pool_transfers if t.name == PoolName.MAMBA
                )
                slots = mamba_transfer.host_indices.cpu()
                mamba_cached_parts.append(slots)
                for device, host, expected in mamba_checks:
                    expected[slots, :, 0] = originals[id(device)][1][
                        :, 2 + start
                    ].unsqueeze(0)
                    assert torch.equal(host.view(torch.uint8), expected), (
                        "KDA backup changed state or untouched slots"
                    )
        ...
        if hybrid:
            extras.append(
                PoolTransfer(PoolName.MAMBA, host_indices=torch.cat(mamba_cached_parts))
            )
        ...
        completion.finish_event.synchronize()
        for device, _, _ in mamba_checks:
            expected = torch.full_like(originals[id(device)][1], 173)
            expected[:, [10, 12]] = originals[id(device)][1][:, [2, 6]]
            assert torch.equal(device.view(torch.uint8).cpu(), expected), (
                "KDA restore changed state or untouched slots"
            )
        for pool in (target, *drafts):
